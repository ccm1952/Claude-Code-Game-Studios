...
> Last updated: 2026-05-14 night continue (Session 33 — **S6-14 story-005 chapter-1-scene-wiring Phase 0 R2 verify ✅ DONE + Phase 1 /story-readiness gate ✅ READY (5 gap closure amend) + Phase 2 scene wiring partial ✅ DONE (unity-mcp batch_execute; AC-1~AC-10 全 ✅ + AC-11 revised per [A2] sub-decision child Hitbox2D GameObject narrow scope 同存解；0 production C# diff + 0 ProjectSettings diff；Drift D Phase 2 surface Unity engine 同 GameObject 2D/3D 互斥约束 BLOCKED [A] 直接共存路径；dp18 sub-item 2 NEW 第 2 实战触发 'Unity engine cross-component 互斥约束 R2 verify 漏'；spike + R3 PlayMode + evidence doc + Phase 5 closure 留下 session) — 7/7 R2 evidence + 4 drift verdict A/B/C/D 沉淀**) — **Drift A** InteractableObject Inspector 字段 spec 误列 _shadowCasterGroup/_renderer 不存在 + 漏列 _dragDepth/_gameplayCamera 必填 (实际 4 项 InteractableObject.cs:44-56) + **Drift B** Layer 注册 spec 写 Layer 9 'Interactable' 实际 Layer 8 'InteractableObject' 已注册不需 add (TagManager.asset:19) + **Drift C** Tag policy spec 'InteractionCoordinator' tag 不存在；用 Untagged + **Drift D** 重大架构 dimensional mismatch — Sprint 2 SP-013 InteractionCoordinator.RaycastWithFatFinger 用 Physics2D.OverlapCircleAll 2D API vs scene Object_01 BoxCollider + Object_02 CapsuleCollider 全 3D collider；5 sprint 累积 architectural drift 未被任何 R2/R3/manual playtest 揭穿 (Sprint 2 EditMode test 用 Physics2D mock fixture 自洽 + Sprint 5 S5-01 scene build 用 3D primitive + S6-13 dp15 sniff sub-clause 验 'production caller > 0' 但不验 'dimensional consistency')。**V3.0.1 dp11 第 2 个实战触发 NEW** (Drift A+B+C 同根 sprint backlog placeholder wording drift；与 S6-13 dp11 第 1 个 SP-013 wording drift 同根；sprint-status.yaml 加 dp11 independent watch_list row + 2 个 data_point 显式 trail) + **V3.0.1 dp18 candidate NEW '物理 API 维度 mismatch (2D/3D collider drift)' 第 1 个实战触发** (Drift D 5 sprint 累积 architectural drift；与 dp15 关联 — dp15 connectivity；dp18 dimensional consistency；Sprint 6 retro 议题 8 候选评估 promote V3.x sub-version + ADR-029 V3 R3 standard amend 'dimensional consistency sniff sub-clause')。**user 决策 (Drift D) [A] narrow scope** Object_01 加 BoxCollider2D + Object_02 加 CircleCollider2D 同存解 (0 production C# change；3D collider 保留给 ShadowMatch ADR-012 可能需要) + **(Drift A/B/C) [1] 原位 amend** story-005 关键 § (Status header / Goal Flow T1-T3 / ADR Decision Summary / Engine Notes R2.1-R2.7 / Control Manifest Rule References / Acceptance Criteria 含 AC-11 NEW 2D collider verify / R3 PlayMode Probe Plan 5+1 case 含 P5 RaycastFatFingerDimensionalConsistency NEW / R2 Assumptions Validated 表 / V3.0.1 Watch List Hooks + dp11 第 2 + dp18 NEW / Implementation Notes 文件清单 / History entry append)；sprint-status.yaml S6-14 status: draft → phase-0-r2-done-pending-readiness-gate + notes 4 drift trail + depends_on 加 dp18 marker + top updated 同步 + dp11 watch_list NEW row 2 data_point (第 1 S6-13 + 第 2 S6-14) + dp18 watch_list NEW row 第 1 data_point。Phase 0 投入 ~30 min。Status: Draft → ✅ Phase 0 R2 verify DONE；**3 file amend done + ReadLints pending + 暂不 commit (per [D] 决策留到 /story-readiness gate 后一并 commit)**。**NEXT — /story-readiness gate (R1+R2+R3 verdict 完整)** ~30-50 min 估时 (R1 grep audit forbidden listener pattern + R2 verify revisit confirm + R3 spike 5+1 case Setup/Action/Assert detail draft；R2 已实证 verdict ⚠️ DEFICIENCY-FLAGGED PASS (Drift D path narrow scope 同存解；如 future story-007 同根则 promote)；R3 verdict ✅ PASS 走 deficiency-flagged path)。 — earlier — Session 32 **Phase 5 closure ✅ DONE — S6-13 story-004 input-pipeline-wiring ✅ DONE Track F NEW 第 1 story / VS Chapter 1 epic 进度 6/10 ✅**) — R3 第 2 跑 **5/5 case PASS + 27/27 asserts + all_passed=true + unexpected_error_count=0 + total_elapsed_ms=391 ≪ 5s budget**；**V3.0.1 dp15 sniff sub-clause 试点 第 1 个 production caller hit > 0 修复 case PASS ⭐** (Sprint 6 retro 议题 6 dp15 promote V3.x sub-version 决策依据 第 1 PASS data point；终极 confirmation 待 story-008 final pilot)；**V3.0.1 dp16 closure 第 1 个实战触发** (ADR-010 §Implementation Guidelines Step 9 NEW "Editor Mouse Adapter" ~25 行 spec wording 落档)；**V3.0.1 dp11 closure 第 1 个实战触发** (sprint-status.yaml Sprint 2 SP-013 status amend "DRIVER PARTIAL closed by S6-13 ✅ Driver 补齐")；**V3.0.1 dp17 candidate discussed and declined** per problem-to-rule-promotion 协议 (user [B] decline 沉淀 trail-only entry — rationale niche)；3 项 Phase 2 deficiency 全 CLOSED ✅；Phase 4 evidence doc `production/qa/playmode-input-pipeline-wiring-2026-05-14.md` NEW ~500 行 8 sections；Phase 5 closure files (5 amend): story-004 Status: DEFICIENCY-FLAGGED PASS → ✅ Done + Completed: "2026-05-14" + History entry append + EPIC.md story-004 row Status: Draft → ✅ Done + epic Status header amend (5 done → 6 done) + Sprint 6 Schedule story-004 entry update + sprint-status.yaml S6-13 status → ✅ done + dp11/dp15/dp16/dp17 declined watch_list trail row append + 顶层 updated 同步 + 本 active.md 同步 + commit (no push)。**governance debt 第 4 次连续 [A] override CLAUDE.md ≤5 hr hard rule** (S6-04 [C] / S6-08 [E] / 本 session readiness gate + Phase 2 + R3 fix + Phase 4 + Phase 5 + commit ≈ ~5-5.5 hr 触达 5hr hard rule 边界) — **Sprint 6 retro 议题 5 STOP THE LINE 强制 actionable resolution 讨论必上**: (a) hard rule wording amend (≤5 hr → ≤6 hr 或 case-by-case 软规则) (b) hard rule override governance trail standard (每次 override 必 explicit user signoff + commit message 强制 'governance debt' marker + retro 议题强制添加 — 本 session 已 follow precedent) (c) hard rule override 阈值 (≥3 次连续 override → 强制 retro stop the line — 本 session 第 4 次远超阈值；retro 必 actionable resolution)。**Track F NEXT story-005** chapter-1-scene-wiring (~1-1.5 hr Asset / Integration — Chapter_01_Approach.unity 加 InteractionCoordinator GameObject + Object_01/02 InteractableObject MonoBehaviour 挂载 + Inspector wire；ADR-013 §Architecture + ADR-027；5-8 AC；R3 N/A reasoned Asset Type)；Track F 余下 4 stories 总估时 ~4-6 hr (story-005 ~1-1.5 hr + story-006 ~30 min + story-007 ~2-3 hr scope 风险最高 R2.1 verify 关键 unknown + story-008 ~1 hr final pilot dp15 sniff sub-clause)；replay manual playtest after Track F done (block S6-02 playtest 2 + S6-03 + S6-10 stage advance)。 — earlier evening Session 32 R3 第 1 跑 FAIL diagnostic + SuspendTick fix — R3 第 1 跑 P1/P3/P4/P5 FAIL (P2 PASS 伪假象) 根因 = InputService.Tick 注册 Utility.Unity.AddUpdateListener 每帧自动跑，spike TickForTest 注入 Began 后 await DelayFrame(1) 期间 production Tick 调 MouseToTouchAdapter.SampleMouse() 拿真实 mouse 当前位置 → SingleFingerFSM.ProcessPending 不查 IsActive 直接计算 frameDist > DragThresholdPx 30.12 → 强制 Pending → Dragging emit Drag.Began (3 phase) 污染所有 Tap case。**SuspendTick fix 3 改动**: (1) InputService.cs +`_tickSuspended` field + `SuspendTick()/ResumeTick()/IsTickSuspended` public API + Tick() early return guard + Dispose() reset (2) S6-13 spike RunAllAsync 起步 `input.SuspendTick()` + try/finally `inputForResume.ResumeTick()` (异常路径也 resume 保护 manual playtest) (3) S6-13 spike P2 `tapZeroOk` 改 baseline delta (`tapDelta = _allTaps.Count - tapBaseline; tapZeroOk = tapDelta == 0`) 防 P1 PASS 后 P2 sequencing 误判 FAIL；ReadLints clean ✅。 — earlier evening Session 32 Phase 2 implementation — S6-13 story-004 input-pipeline-wiring Phase 2 production code ✅ DONE batch write 7 step (~50-70 min); ReadLints clean ✅; Files (8 changed, 3 NEW + 5 amend): (1) NEW `Assets/GameScripts/HotFix/GameLogic/Input/InputService.cs` ~155 行 (TEngine listener-path driver pattern matching SceneManager — plain class + Init/Dispose lifecycle + Utility.Unity.AddUpdateListener(_tickCached) Tick loop + #if UNITY_EDITOR Tick branch 走 MouseToTouchAdapter → SingleFingerFSM.Update → GestureDispatcher.Dispatch + Player Build #else explicit empty branch (Sprint 7+ Touch real device 接入入口) + TickForTest reflection 入口 R3 spike 用 + InputConfigFromLuban.InitWithDefaults() Sprint 6 GDD defaults sufficient) + (2) NEW `Assets/GameScripts/HotFix/GameLogic/Input/MouseToTouchAdapter.cs` ~75 行 ENTIRE FILE #if UNITY_EDITOR guard (Mouse Button 0 → TouchState Phase=Began/Moved/Stationary/Ended/Canceled per Touch lifecycle 一致 + 0 GC alloc 仅 _wasDown/_lastPos 2 field state) + (3) `GameApp.cs` amend (+15 行) — `_inputService` private static field + Entrance() 内 SceneManager block 之后 InputService new+Init + Release() 内 InputService.Dispose() 必须先于 SceneManager Dispose (Utility.Unity.RemoveUpdateListener 需 update driver alive) + RegisterDevSpikes() S601PlaytestSpike → S613Spike 切换 (S6-01-playtest 注释行保留作 manual playtest 模式切换入口) + (4) `DevTestState.cs` amend [main-menu] mode HasSpike list +1 = 6 spike (S5-02 + S6-07 + S6-08 + S6-04 + S6-01-playtest + **S6-13**) — V3.0.1 dp8 candidate 阈值远超达 (>4) Sprint 6 retro 强制评估 V3.1 trigger central mode-dispatch refactor + (5) NEW `Assets/GameScripts/HotFix/GameLogic/DevTest/Spikes/S6-13_InputPipelineWiring.cs` ~430 行 (1 file + 3 inner class S613Spike : IDevSpike + S613Runtime : MonoBehaviour + S613Tester 纯逻辑 沿用 S6-04/S6-08 precedent — 5 R3 case Setup/Action/Assert P1 MouseSingleTapToOnTapEvent 6 asserts + P2 MouseDragThreePhaseFire 5 asserts + P3 TapVsDragThresholdBoundary 6 asserts + P4 SingleFingerFSMStateTransitionVerify 4 asserts + P5 NoMockFireBypassVerify 6 asserts dp15 sniff sub-clause 试点 第 1 个 production caller hit > 0 修复 case verify + listener spy IGestureEvent_Event.OnTap/OnDrag GameEvent.AddEventListener<GestureData> + Application.logMessageReceived UnexpectedErrorCount + ExpectedLogSubstrings 8 pattern allowlist + GetProductionInputService reflection helper (拿 GameApp._inputService private static field) + InputService.TickForTest reflection 入口注入 TouchState (绕 MouseToTouchAdapter Mouse hardware 依赖) + JSON evidence dump WriteResultJson per Application.persistentDataPath/S6-13_Result.json + OnGUI overlay 5 case status row) + (6) `docs/architecture/adr-010-input-abstraction.md` §Implementation Guidelines amend Step 9 "Editor Mouse Adapter" ~25 行 spec wording 落档 (V3.0.1 dp16 candidate "ADR spec gap re Editor-only path" closure 第 1 项) — Adapter 不进入 production runtime build #if UNITY_EDITOR guard + InputService.Tick #else branch Sprint 7+ Touch 真机 testing 接入入口 + Mouse Phase 翻译规则 Down→Began/Held→Moved|Stationary/Up→Ended/Idle→Canceled + 0 GC alloc hot path + 关联实施 reference 3 文件 + § Last Verified amend 2026-05-14 + (7) Step 7 SP-013 wording drift 实证 — sprint-status.yaml 当前 active sprint 6 stories 中**SP-013 entry 实际不存在** (Sprint 2 用 S2-XX 编号 / SP-XXX 是 Spike 编号 / SP-011 是 YooAsset Additive Spike) — story-004 引用的 "Sprint 2 SP-013" wording **本身就是 V3.0.1 dp11 candidate "sprint backlog placeholder wording drift" 的实战首次触发例**；watch_list dp11/dp15/dp16 candidate row 已在 readiness gate 阶段沉淀 (sprint-status.yaml line 1779-1794)；**Step 7 amend redirected** = sprint-status.yaml watch_list 内 dp11 closure trail 在 Phase 5 closure 时同步沉淀 + commit message 反映 actual SP-013 wording drift 实证 + sprint-status.yaml top updated 同步 + 本 active.md 同步 + (8) ReadLints clean ✅ 0 lint over 5 file (InputService.cs + MouseToTouchAdapter.cs + S6-13_InputPipelineWiring.cs + GameApp.cs + DevTestState.cs)。**Phase 2 总投入 ~50-70 min** (Step 0 references read ~10 min + Step 1 InputService.cs ~10 min + Step 2 MouseToTouchAdapter.cs ~5 min + Step 3 GameApp.cs amend ~5 min + Step 4 S6-13 spike ~25 min + Step 5 RegisterDevSpikes amend ~3 min + Step 6 ADR-010 amend ~5 min + Step 7 SP-013 wording drift 实证 ~2 min + ReadLints + active.md 同步 + commit ~5 min)；本 session 总投入达 ~3.5-4 hr (Track F batch ~50 min + readiness gate ~50 min + amend ~30 min + Phase 2 ~50-70 min) — **第 3 次连续 [A] override CLAUDE.md ≤5 hr hard rule 暂仍合规 (本 session 实际 ≈ 4 hr 边界内)**；governance debt accumulated trail Sprint 6 retro 议题 5 input。**NEXT — Phase 3 Step 1 ReadLints + Unity Editor compile verify (await domain reload) + Phase 3 Step 2 R3 PlayMode 5/5 case execute + JSON evidence dump 验证** (用户在 Unity Editor 启动 PlayMode 跑 spike 完成 R3 case verify；预期 P1~P4 PASS + P5 dp15 sniff sub-clause 第 1 个 production caller hit > 0 修复 case verify + JSON 落 ~/Library/Application Support/DefaultCompany/Unity/S6-13_Result.json) → Phase 4 evidence doc `playmode-input-pipeline-wiring-2026-05-14.md` ~400 行 8 sections (per S6-04/S6-08 precedent) → Phase 5 closure (story-004 Status: DEFICIENCY-FLAGGED PASS → ✅ DONE + sprint-status.yaml + active.md + commit). — earlier afternoon Session 32 readiness gate continue — **S6-13 story-004 input-pipeline-wiring Phase 0/1 R1+R2+R3 readiness gate ✅ DEFICIENCY-FLAGGED PASS (~45-60 min)** — R1 ✅ PASS (forbidden listener pattern grep audit 7 项全 compliant) + R2 ⚠️ DEFICIENCY-FLAGGED PASS per ADR-029 V2.0 §V2-1 path (6/8 ✅ + 2/8 ⚠️ PARTIAL R2.1 Sprint 2 SP-013 partial (FSM 层 ✅ 10 file + Driver 层 ❌ 0 — InputService.cs 不存在 + Touch/Mouse Update Loop 0 — V3.0.1 dp11 同根 sprint backlog placeholder wording drift candidate 实战首次触发) + R2.7 Luban TbInputConfig pending + 1/8 ⚠️ NEW DEFICIENCY R2.5 ADR-010 spec gap re Editor Mouse pipeline (V3.0.1 dp16 NEW candidate 'ADR spec gap re Editor-only path' 实战首次触发 与 dp15 同根但深一层 — dp15 production runtime caller R3 sniff；dp16 ADR spec Editor-only path R2 grep verify)) + R3 ✅ PASS (5 case Setup/Action/Assert detail P1 MouseSingleTapToOnTapEvent + P2 MouseDragThreePhaseFire + P3 TapVsDragThresholdBoundary + P4 SingleFingerFSMStateTransitionVerify + P5 NoMockFireBypassVerify dp15 sniff 试点 第 1 个 production caller hit > 0 修复 case + JSON schema spec consistency + stub type construction signatures 全 production-ready)；3 项 Phase 2 deficiency closure: (1) ADR-010 §Implementation Guidelines amend Step 9 'Editor Mouse Adapter' ~5-10 行 spec wording (dp16 closure 第 1 个) + (2) sprint-status.yaml Sprint 2 SP-013 status amend 'DONE → partial-fsm-only-driver-pending-sprint-6-emergent-fix' (dp11 closure 第 1 个) + (3) story-004 wording drift propagate fix 残留 review (本 amend 已修正大部分 'GestureRecognizer FSM' / 'ScreenPos' wording)；story-004 amend 13 sections (Status header DEFICIENCY-FLAGGED PASS + Depends on partial marker + §Context dp11 同根 candidate 段 + §Goal Flow T0~T6 wording propagate fix MouseToTouchAdapter + SingleFingerFSM + ScreenPosition + §ADR Decision Summary 加 Phase 2 deficiency closure list 3 项 + §Engine Notes R2.1~R2.8 verdict 实证 update 含 Phase 2 production wiring file list 6 项 + §Control Manifest Rule References ⚠️ TBD → ✅ verified 7 项 + §Acceptance Criteria 7→10 (AC-8/-9/-10 deficiency closure verification) + §R3 PlayMode Probe Plan 5 case Setup/Action/Assert detail + §R2 Assumptions Validated 表 + §V3.0.1 Watch List Hooks **Type-11 dp16 NEW candidate** 触发 + §Out of Scope 加 PC Mouse pipeline + DualFingerFSM + InputBlocker listener-side + Luban TbInputConfig 4 项 + §Implementation Notes 8 项文件清单精化 + §History entry append)；sprint-status.yaml S6-13 status: draft → phase-1-readiness-gate-done-deficiency-flagged + watch_list dp16 NEW row append (Type-11 candidate 'ADR spec gap re Editor-only path') + 顶层 updated 同步；本 amend 投入 ~50 min；Phase 2 implementation ~3-5 hr (修订原 ~2-3 hr 估时因 Sprint 2 SP-013 实际 partial)。 — earlier afternoon batch write continue **emergent fix epic Track F NEW vs-chapter-1 batch write 5 stories Draft (story-004 input-pipeline-wiring + story-005 chapter-1-scene-wiring + story-006 gameapp-provider-injection + story-007 shadowmatch-production-wire + story-008 end-to-end-smoke-replay) ✅ + EPIC.md amend Stories table +5 row + Governing ADRs +ADR-010/-012/-013 + Sprint 5+6 Schedule rename + 风险表 +4 row ✅ + sprint-status.yaml +5 entries S6-13~17 (ID 冲突修复 Nice to Have S6-11/S6-12 已占用) ✅** — earlier morning continue **S6-01 Phase 2.1 manual playtest NEEDS-WORK closure ⚠️ + V3.0.1 dp15 candidate NEW 'EditMode green ≠ production wired sniff' (promote 优先级 高于 dp14) + Sprint 6 emergent fix epic Track F NEW vs-chapter-1 story-004~008 plan ~7-9 hr** — Phase 2.1 manual playtest 启动 ~5 min 后 user 报 'Object_01 无法交互, 点击拖拽 没响应'；agent root cause analysis (rg + Session 27 #2 R2.3 + unity-mcp 实测交叉验证) surface chapter 1 production wiring **5 处 S0 critical 缺口同时存在**：(S0-1) Touch/Mouse → GestureDispatcher.Dispatch production caller 0 hit (`rg 'GestureDispatcher\.Dispatch' --type cs` 仅 EditMode GestureDispatchTests.cs 1 处) → IGestureEvent.OnTap 在 production 0 sender；(S0-2) Chapter_01_Approach.unity 未挂 InteractionCoordinator GameObject (Session 27 #2 unity-mcp 实测无此节点)；(S0-3) Object_01_CoffeeMug / Object_02_Book 缺 InteractableObject MonoBehaviour (Session 27 #2 unity-mcp 实测 componentTypes [Transform, MeshFilter, BoxCollider, MeshRenderer]；当时决策推迟 Sprint 6 polish 但 Sprint 6 stories 未排此项)；(S0-4) GameApp.Entrance 0 调 InteractableObject.RegisterPuzzleConfigProvider + InteractionCoordinator.RegisterInputConfigProvider；(S0-5) ShadowMatchCalculator (ADR-012) listener production 0 wire (S5-02 spike P2 fire mock OnMatchScoreUpdated(1, 0.95) 简化路径绕过)。**Sprint 2 → Sprint 5 五个 sprint 期间 ObjectInteraction system 整套 EditMode test pass + production 0 wiring 反模式直接被 manual playtest 5 min 内穿透** — Sprint 2 ObjectInteraction system 代码齐全 (5 production class — InteractableObject + InteractableObjectFsm + InteractableObjectFeedback + InteractionCoordinator + InteractionLockManager + EditMode test 全 pass)，但 chapter 1 production wiring 整套 0 落地；S5-02 spike fire mock 简化路径让 R3 5/5 PASS 掩盖整套 wiring gap，未被任何 R2 / R3 / smoke check 揭穿。**V3.0.1 dp15 candidate NEW (promote 优先级 高于 dp14) 'EditMode green ≠ production wired sniff'** — proposed ADR-029 V3 R3 standard amend "production caller hit > 0 sniff sub-clause" (`rg 'XxxDispatcher\.Yyy' / 'class XxxManager.*MonoBehaviour'` production caller > 0 = sniff PASS；= 0 仅 EditMode test fixture 调 → 黄牌警告 spike 路径可能 mock 绕过 wiring gap，强制评估补 R3 production wiring sniff case)；Sprint 7+ 影响：适用于全部跨 sender → listener 链路系统 (ShadowMatchCalculator / NarrativeSequencePlayer / ChapterStateManager / 未来 InputManager epic 等)。**Verdict NEEDS-WORK** (5 处 S0 触发 NEEDS-WORK criteria；fun loop 不可评估 — 不是评估出来 fail 是评估前提（chapter 1 物件可交互）不存在；governance value 满足 — playtest 1 设计意图 = 揭露 Sprint 6 polish 优先级达成)；**ADR-030 §VS Build commit 第 1/3 partial done** (fun loop ≥30 min 不达，但 governance value 巨大 surface dp15 NEW + 5 处 wiring gap → emergent fix 触发；replay after story-004~008 done 后再判定是否需重计 1/3)。**Emergent fix epic Track F NEW vs-chapter-1 story-004~008 plan ~7-9 hr Sprint 6 本周 split 2-3 daily session 嵌入** (沿用 production/epics/vs-chapter-1/ epic 加 story-004~008 不另起 epic)：story-004 input-pipeline-wiring ~2-3 hr (Touch/Mouse → GestureRecognizer FSM → GestureDispatcher.Dispatch production wire；ADR-010 Input Abstraction 部分 scope pull-forward 到 Sprint 6 governance justification = block S6-02 playtest 2 + S6-03 + S6-10 gate-check pre-production stage 推进；AC: Editor Play 时 Mouse click 能 fire IGestureEvent.OnTap + PlayMode probe 1 case Mouse Tap → event round-trip) + story-005 chapter-1-scene-wiring ~1-1.5 hr (Chapter_01_Approach.unity 加 InteractionCoordinator GameObject + Inspector 拖入 _objects + Camera + Layer + Object_01/02 加 InteractableObject MonoBehaviour + 配 _puzzleId + Interactable layer；ADR-013 Object Interaction FSM + ADR-027；AC: unity-mcp manage_scene get_hierarchy 验组件齐全 + Object layer = Interactable) + story-006 gameapp-provider-injection ~0.5 hr (GameApp.Entrance 调 InteractableObject.RegisterPuzzleConfigProvider + InteractionCoordinator.RegisterInputConfigProvider；ADR-013 §Architecture + Sprint 2 SP-013 注入约定；AC: GameApp.cs 见 2 处 Register + PlayMode probe 启动后 InteractableObject._puzzleConfig != null) + story-007 shadowmatch-production-wire ~2-3 hr (ADR-012 ShadowMatchCalculator listener 物件 transform → 阴影匹配 → IPuzzleEvent.OnMatchScoreUpdated fire production 不再 spike fire mock；AC: 拖动 Object_01 接近正确位置 → score 变化 + score=0.95 fire OnPerfectMatch → narrative trigger manual playtest replay 验) + story-008 end-to-end-smoke-replay ~1 hr (S5-02 spike 重写：去除 mock fire OnMatchScoreUpdated 改用 production wiring simulate Tap+drag → match → score → narrative 完整 round-trip；ADR-029 V3 R3 standard + V3.0.1 dp15 candidate sniff sub-clause 试点；AC: R3 1 case 验 production wiring 不依赖 mock 也能 happy path 走通)。**S6-09 art asset 升级时机决策**: 推 [A2] (playtest 1 反馈不足以判定 art 是否 fun loop 阻碍 → emergent fix done 后 replay session 再评估)；**Sprint 6 retro 议题 +1=6**: NEW 议题 6 dp15 candidate 'EditMode green ≠ production wired sniff' (promote 优先级 高于议题 5 dp14；与 dp14 关联评估同根 'spike/test PASS 不等于 production 可玩'反模式但层级不同 — dp14 manual playtest 工具链 gap 涉及 Phase 1 prep，dp15 EditMode-vs-production 验证可信度 gap 涉及 R3 standard 自身)。Files (3 amend): playtest-vs-chapter-1-session-1-2026-05-13.md amend §0 verdict NEEDS-WORK + §2.3.1 物件探索 player notes user 原话 + 5 wiring gap root cause 表 + §3 Fun Loop Assessment 全 不可评估 + §4.1 S0 critical 5 项 NEW + §4.5 dp15 candidate NEW 段 (promote 优先级 高于 dp14) + §5 ADR-030 §VS Build commit partial done + §6.2 emergent fix epic Track F NEW story-004~008 表 + §6.4 retro 议题 +1=6 + §8 Sign-off NEEDS-WORK + sprint-status.yaml (本 entry top updated + S6-01 status: phase-2-0-infra-fix-done → phase-2-1-played-needs-work-emergent-fix-pending + watch_list dp15 NEW row append (Type-11 candidate 'EditMode green ≠ production wired sniff') + 本 active.md。Earlier Session 32 morning — **S6-01 Phase 2.0 playtest infrastructure fix ✅ DONE** — Phase 2 起步时 surface **V3.0.1 dp14 candidate NEW (Type-10) 'playtest infrastructure pattern gap'** (S6-01 Phase 1 prep 盲点 — 所有现有 DevSpike 是 R3 PlayMode test spike Launch() 内 AddComponent MonoBehaviour → Awake/Start 自动 RunAllAsync 跑 R3 case + assert，与 manual playtest 节奏完全不兼容)；起步发现路径: user fresh Unity boot Step 1 报 'Build Settings 没有 main menu 场景, 只有 main scene' → agent verify 发现 main menu 是 UIWindow (MainMenuPanel.prefab) 而非 scene (S6-07 polished), main.unity 是 TEngine vendor 默认 boot scene → 即问题在于 GameApp.RegisterDevSpikes 当前注册 S604Spike → S6-04 spike RunAllAsync 自动跑 5 R3 case (fire OnRequestSceneChange(99) UnknownChapter / fire(2) chapter switch / Error state recovery 等) 完全打断用户手动 click NewGame 进 chapter 1 节奏。Fix 路径 [A] PlaytestSpike NEW (~15-20 min): (1) 新建 `Assets/GameScripts/HotFix/GameLogic/DevTest/Spikes/S6-01_PlaytestHoldMode.cs` ~50 行 (S601PlaytestSpike : IDevSpike, Id='S6-01-playtest', Name 描述 manual playtest infrastructure, Launch() no-op 仅 Log.Info 占位) (2) `DevTestState.cs` `[main-menu]` mode HasSpike list +1 (S5-02/S6-07/S6-08/S6-04/**S6-01-playtest**=5 远超原阈值 4 — V3.0.1 dp8 candidate +1=5 冗余触发) (3) `GameApp.cs` RegisterDevSpikes 切换 S604Spike → S601PlaytestSpike (S6-04 注释行保留作 R3 复跑入口) (4) ReadLints clean ✅ 0 lint。Governance trail: evidence doc §4.5 dp candidate NEW dp14 + §6.4 retro 议题 NEW 议题 5 + Status Log Phase 2.0 entry + sprint-status.yaml Type-10 candidate dp14 NEW row append (sprint_5_adr_029_v3_watch_list — V3 watch list 吸收新 V3.0.1 dp candidate 模式) + S6-01 entry status: phase-1-prep-done → phase-2-0-infra-fix-done + file 字段 amend Phase 2.0 marker。Sprint 6 retro 议题 +1=5: dp14 与 dp8 关联评估是否合并 V3.1 trigger 提案 (DevTestState central mode-dispatch refactor + standard PlaytestMode pattern doc 双成果)。Sprint 7+ 复用: S6-02 / S6-03 / 未来 chapter 2 playtest 等所有 manual playtest session 复用此 spike pattern。Phase 2.0 投入 ~15-20 min；**NEXT — Phase 2.1 manual playtest ≥30 min** user fresh Unity boot 应能看到 main menu 显示 + 用户手动 click NewGame 进 chapter 1 (R3 spike auto run 不再干扰). Earlier Session 31 — **S6-01 Phase 1 prep ✅ DONE** — `production/playtests/` 目录创建 + `playtest-vs-chapter-1-session-1-2026-05-13.md` 8 sections full template 落盘 (§0 概要 — chapter 1 第 1 次 internal playtest S5-07a carryover from Sprint 5 + ADR-030 §VS Build commit 第 1/3 次 + Phase 1 prep status flow + 6 Acceptance criteria + §1 Setup Unity Editor 2022.3.62f2 + Chapter_01_Approach.unity + Phase 2 起步 6 步 checklist + §2 Player Journey 5 阶段 detailed log placeholder — splash→menu / menu→ch1 / fun loop 5 子区 物件探索 + shadow match + puzzle state + narrative+audio duck + 5 systems 串通整体感觉 / ch1 end / robustness manual + §3 Fun Loop Assessment 6 维度 1-5 评分表 fun moment 频次 + pacing + clarity + frustration + 想再玩意愿 + 整体健康度 + §4 Emergent Issues 4 级 S0/S1/S2/S3 + dp candidate slot governance drift + §5 ADR-030 §VS Build commit 6 项 progress (1+2 ✅ done / 3 ⏳ 1/3 after 本 session / 4+5+6 ⏳ Sprint 6 末) + §6 Sprint 6 后续影响 S6-09 art asset 升级决策 [A1/A2/A3] + emergent fixes Sprint 6 vs Sprint 7 分配 + Playtest 2 起手前置 + Sprint 6 retro 议题 link + §7 References 前置依赖 evidence (S5-02/S6-04/S6-07/S6-08/S5-04) + Governance + Asset + §8 Sign-off PASS (≥3.5 均分 + S0=0 + S1 ≤3) / NEEDS-WORK / FAIL criteria + Appendix A Cursor observer playbook 4 类辅助 (实时提醒 / Console error 分类 / 反馈实时记录 / 玩后整理))；sprint-status.yaml S6-01 entry status: ready-for-dev → phase-1-prep-done + depends_on 全 ✅ marker + file 字段从 `2026-05-XX placeholder` → `2026-05-13`；总 Phase 1 prep 投入 ~30 min；**Phase 2 起步 prerequisite**: user fresh Unity Editor boot (关掉重开避免 S6-04 R3 PlayMode 残留 state 污染 reuse session 语义) + 计时器开 + Console error 级别独留 + ≥30 min continuous play 不中断 + 边玩 Cursor side observer 实时记录反馈。Earlier Session 31 — **Upstream Sync v1.0.0 ✅ merged 本地 + 0 push** — Donchitos/Claude-Code-Game-Studios v1.0.0 release 2026-05-13 全部同步; merge template/main 8 commits / 104 files / +2972/-743; **1 conflict resolved** (dev-story Phase 5 hybrid merge — 保留 ShadowGame Unity asmdef rule for TEngine Roslyn Source Generator GameEventHelper.g.cs CS0246 防范 + 采纳上游简化 Phase 5 + Phase 4 path-only brief); **Auto-merge 4 文件 clean** (CLAUDE.md "48→49 coordinated subagents" + session-start.sh fix #43 head→tail 显示最新 state + create-stories + story-readiness ADR-029 V3 propagation 保留); **Phase C 15/15 sanity check 全过** (73 skills + 49 agents + ADR-029 V3 propagation 完好 [dev-story Phase 1.5 R1/R2/R3 + story-readiness verdict dimension] + Unity asmdef rule 保留 + ShadowGame Unity 2022.3.62f2 + TEngine + HybridCLR + YooAsset stack 完整 + 0 lint); main HEAD = `0322ea3` ahead origin/main 6 commits **待 push** (PAT workflow scope 缓存); backup branch `upstream-sync-2026-05-13` 暂留 (验证 OK 后删). **Phase C 留 Sprint 6 retro 处理**: (1) `/vertical-slice` (Pre-Production gate, 上游 v1.0 新加 359 行 skill) vs S6-10 `/gate-check pre-production` sequencing 评估 (推测: `/vertical-slice` 先跑产 vertical-slice-report.md → `/gate-check` 检查 artifact); (2) **dp13 候选** Director gates 系统 (CD-/TD-/LP-/AD- prefixes, 上游 v0.4.x→v1.0 引入) vs ADR-029 V3 governance maturity coexistence — 决定另起 ADR-031 vs amend ADR-029; (3) `/prototype` 大改造 (双模式 concept Phase 1 vs vertical Phase 4) ADR-030 §VS Build commit 是否需补充). Earlier Session 30 continue — **S6-04 ✅ DONE Phase 2+3+4+5 一气呵成完成 — Sprint 6 Track C 100% 收官 + VS Chapter 1 epic 100% 完成 ✅**) — **R3 PlayMode 第 2 跑 5/5 case PASS / 37/37 asserts / all_passed=true / unexpected_error_count=0 / total 1867ms < 8s budget**。第 1 跑 P4/P5 FAIL 暴露 **V3.0.1 NEW dp12 candidate** (isolated local SceneManager + global GameEvent collateral 首次实战) — P3 isolated local sm fire(99) 是 global event-path → production sm collateral TryResolveOrFail FAIL 进 Error → P3 cleanup 只 local.Dispose() 漏 production sm RecoverToIdle 致 P4/P5 precondition (state=Idle) FAIL；Phase 3 spike amend `RunP3Async(SceneManager productionSm)` 加 collateral RecoverToIdle cleanup → 第 2 跑 5/5 PASS；ADR-009 V3.x watch list 候选。**Phase 2 spike code**: `Assets/GameScripts/HotFix/GameLogic/DevTest/Spikes/S6-04_ErrorRestartPath.cs` NEW ~620 行 (1 file + 3 inner class S604Spike/S604Runtime/S604Tester + 5 R3 case P1 UnknownChapterTryResolveOrFail + P2 NewestWinsPendingDuringTransition + P3 AssetLoadFailRetryExhaust + P4 RestartFromErrorRecovery + P5 RapidNewestWinsOverwrite + 5 ISceneEvent listener spy + Application.logMessageReceived UnexpectedErrorCount + ExpectedLogSubstrings 19 pattern allowlist + GetProductionSceneManager reflection + isolated local SceneManager P3 case 隔离 + JSON evidence dump WriteResultJson) + GameApp.cs RegisterDevSpikes S608Spike → S604Spike 切换 + DevTestState.cs `[main-menu]` mode HasSpike(S6-04) +1 = **4 distinct spike** (S5-02 + S6-07 + S6-08 + S6-04 — V3.0.1 dp8 candidate **阈值 4 触达** — Sprint 6 retro 强制评估 promote V3.1 trigger pattern DevTestState central mode-dispatch refactor 候选)。**Phase 4 evidence doc** `production/qa/playmode-error-restart-path-2026-05-13.md` ~400 行 8 sections (§0..§8 per S6-08 precedent — 概要 + 5 case detail + R2 7/7 closure + AC 10/10 verify + V3.0.1 Watch List Hooks dp8 阈值触达 + dp11 closure + dp12 NEW first-strike closure + Sprint 6 Track C 收官 insight + Files changed + References + Verdict)。**Phase 5 closure**: story-003 Status: Phase 1 ready → ✅ Done + EPIC.md story 003 row Status: Done + Sprint 5 Schedule story-003 update + sprint-status.yaml S6-04 phase-1-readiness-gate-done → ✅ done + top updated + 本 active.md。**R2 verdict 7/7 ✅ FULLY PASS** (R2.7 IGameFlowEvent.OnReturnToMainMenu Sprint 7+ ADR-010/ChapterSelect epic boundary DEFERRED 复用 S6-07/S6-08 precedent — DEFICIENCY-FLAGGED PASS path **第 3 次实战**)。**AC verdict 10/10 PASS** (本 story 0 production code change narrow scope [A] 决策保证 AC 通过 vendor reality 已 production 路径全 cover)。Files (8 changed): S6-04_ErrorRestartPath.cs NEW + GameApp.cs amend + DevTestState.cs amend + story-003 amend + EPIC.md amend + sprint-status.yaml (本 entry + top updated) + 本 active.md amend + production/qa/playmode-error-restart-path-2026-05-13.md NEW。**总 session 投入 ~7-8 hr** (S6-08 Phase 2~5 ~3 hr + S6-04 Phase 0+1 ~1 hr + S6-04 Phase 2~5 ~2 hr) — user 2 次 [C] override CLAUDE.md ≤5 hr hard rule (**governance debt 已记录至本 entry + sprint-status.yaml — Sprint 6 retro 讨论防再发**)。**Sprint 6 Track C 100% 收官** (S6-05 + S6-06 + S6-07 + S6-08 + S6-04 5/5 done)。**VS Chapter 1 epic 100% 完成 ✅** — ADR-030 §VS Build commit 第 1 项 chapter 1 robustness 补全 closure；≥3 playtest sessions prerequisite 100% ready (Track C 5/5 done + Track B 2/2 done — Track B 余下评估 next session)。**Earlier in Session 30 continue — S6-04 Phase 0 + Phase 1 ✅ READY DONE — story-003-error-restart-path.md NEW per V3.0.1 vendor reality compliant + narrow scope [A] 0 production code change**) — Phase 0 R2 vendor reality verify 7 finding 实证: (R2.1) `SceneManagerState.Error` + `RecoverToIdle()` 显式恢复 production (SceneManager.cs:22 enum + line 293-303) (R2.2) `TryResolveOrFail(id)` unknown chapter → `OnSceneLoadFailed(id, "Chapter ID X not found in TbChapter.")` + `TransitionTo(Error)` production (line 248-263 — provider != null 且 provider(id)==null 路径) (R2.3) `LoadChapterSceneAsync` `MaxLoadRetry=2` retry exhaust production (line 74 const + line 476-528 retry loop + line 530-533 exhaust `OnSceneLoadFailed`) (R2.4) AC-9 **newest-wins pending overwrite** during transition production (line 350-352 `_pendingTargetChapterId = newest target` + line 358-385 `DrainPending`) (R2.5) AC-10 Error 状态 silent drop production (line 312-316 + warning) (R2.6) AC-8 Idle + same target silent `OnSceneReady` production (line 329-333 + DrainPending 内同语义 line 365-369) (R2.7) ❌ **IGameFlowEvent.OnReturnToMainMenu 0-production** — 没有 IGameFlowEvent.cs；ISceneEvent.OnRequestSceneChange 仅支持 1..5 (line 24)；MainMenuPanel.Quit = Application.Quit；S5-02 dp6 NEW 已记录此 spec drift — DEFERRED Sprint 7+ ChapterSelect epic 或 ADR-010 InputManager epic 一并 (epic boundary cleanup S6-04 R2.7 **第 3 次实战** — S6-07 R2.6/R2.8 + S6-08 R2.7 + S6-04 R2.7 三连复用 [D] mixed strategy precedent)。**Narrow scope [A] 决策 ~1 SP 0 production code change** (vendor reality verify only)：spike `S6-04_ErrorRestartPath.cs` ~600-700 行 (1 file + 2 inner class + 5 R3 case + listener spy + isolated local SceneManager P3 case 隔离 + JSON evidence dump) + GameApp.cs RegisterDevSpikes S608→S604 切换 (~3 行) + DevTestState.cs `[main-menu]` mode `HasSpike(S6-04)` +1 = **4 distinct spike** (S5-02 + S6-07 + S6-08 + S6-04 — V3.0.1 dp8 candidate **TRIGGERED 阈值 4** Sprint 6 retro 强制评估 promote V3.1 trigger pattern DevTestState central mode-dispatch refactor 候选)。**5 R3 PlayMode probe case** (S5-2b placeholder 2 处 wording drift amend per V3.0.1 vendor reality compliant): **P1 UnknownChapterTryResolveOrFail** (chapter 1 baseline 后 fire 99 → assert OnSceneLoadFailed + state=Error + CurrentChapterId=1 不变) + **P2 NewestWinsPendingDuringTransition** (chapter 1→2 transition 中 fire 1 → newest-wins pending → drain 后 chapter 1 reload；**原 'mid-transition cancel' wording drift amend per AC-9**) + **P3 AssetLoadFailRetryExhaust** (isolated local SceneManager + fixture chapter 99 bad sceneId → 2 retry exhaust + OnSceneLoadFailed) + **P4 RestartFromErrorRecovery** (Error → AC-10 silent drop verify → `RecoverToIdle()` → state=Idle → re-fire(1) success + chapter 1 reload) + **P5 RapidNewestWinsOverwrite** (Idle rapid fire 1→2→1 → 第 1 same target silent OnSceneReady AC-8 + 第 2 进 TransitionOut + 第 3 newest-wins pending AC-9 → drain final chapter 1；**原 'repeated rapid debounce' wording drift amend per AC-9**)。**9 R2 表** (R2.1~R2.6 ✅ FULLY；R2.7 ⚠️ DEFERRED Sprint 7+ epic boundary；R2.8 + R2.9 ⚠️ TBD Phase 2 实施时确认 5 case state cleanup + DevTestState dp8 candidate)。**V3.0.1 Watch List Hooks** dp8 candidate **TRIGGERED 阈值 4** + **NEW dp11 candidate** sprint backlog placeholder wording drift detection (S5-2b → S6-04 rewrite 揭露 2 处 vendor reality wording drift — 留观察单次启动)。Phase 1 R1+R2+R3 readiness gate verdict ✅ **DEFICIENCY-FLAGGED PASS** ready for Phase 2 transition。Files (4): story-003-error-restart-path.md NEW (~480 行) + EPIC.md row 5 amend Status: Sprint 6 backlog → Phase 1 ready + Overview §story-003 description update + sprint-status.yaml (本 entry + top updated) + 本 active.md 同步。Phase 0+1 投入 ~30-45 min (Session 30 continue — 本 session 总时长 ≈ 5-6 hr 经 user [C] override CLAUDE.md ≤5 hr hard rule 一次性完成 S6-04 Phase 0+1 — `discussed and declined ≤5 hr` per protocol)。**Phase 2 production code defer 下一 session 起** per CLAUDE.md ≤5 hr 硬规则 (Phase 2 实施清单：(1) S6-04_ErrorRestartPath.cs spike (2) GameApp.cs RegisterDevSpikes 切换 (3) DevTestState.cs HasSpike(S6-04) +1)。**Sprint 6 Track C 进度 2/3 done (S6-07 ✅ + S6-08 ✅) + S6-04 Phase 1 ready** — S6-04 Phase 2~5 完成后 Track C 100% complete + S6-01 playtest 1 prerequisite 100% ready。**Earlier in Session 30 continue — S6-08 ✅ DONE — Phase 2+3+4+5 完整实施 + R3 PlayMode 5/5 case + 35/35 asserts + all_passed=true 第 2 跑 after V3.0.1 dp9 NEW spec wording amend + Push=8/Pop=8 对称 + 0 unexpected error**) — Phase 2 production code: `UIModule.InputBlocker.cs` NEW partial class ~85 行 (TryFireInputBlockerPush/Pop(UIWindow, fromHideUI=false) helper + `_inputBlockerPoppedByHide` HashSet 防 HideUI delayed close → CloseUI 双 fire + `ShouldFireInputBlocker(UIWindow)` Top=2 || Tips=3 layer filter) + `UIModule.cs` ~+20 行 6 hook entry insert (ShowUIImp Type/T/T Await 3 first-show + TryGetWindow re-show + CloseUI InternalDestroy 之前 + HideUI 真 hide fromHideUI:true) + `S6_08_MockPanels.cs` NEW ~160 行 7 mock UIWindow (MockTopPanel + MockTopPanel2 + MockTipsPanelA/B/C + MockBottomPanel + MockSystemPanel + 各 LastInstance static + OnCreate/OnDestroy hook) + `S6_08_MockPanelsGenerator.cs` NEW ~120 行 Editor MenuItem (Tools/S6-08/Generate Mock Panel Prefabs (All)) + 7 Assets/Resources/UI/Mock*.prefab + `S6-08_PopupAutoBlocker.cs` NEW ~680 行 1 spike + S608Runtime + S608Tester + 5 R3 case (P1 TopLayerSenderVerify + P2 UIBottomSystemNoFire + P3 TipsPopupQueueChain + P4 SortingDepthVerify + P5 PauseResumeClearQueue) + listener spy + reflection helper (`GetCurrentPopupType` + `GetWindowDepth`) + JSON evidence dump WriteResultJson + `GameApp.cs` RegisterDevSpikes S607Spike → S608Spike 切换 + `DevTestState.cs` `[main-menu]` mode 扩 `HasSpike("S5-02") || HasSpike("S6-07") || HasSpike("S6-08")` V3.0.1 dp8 candidate +1 spike count 2→3。**Phase 3 R3 PlayMode**: 第 1 跑 (16:10) P1 ✅ + P2 ✅ + P3 ❌ + P4 ❌ + P5 ❌ → 暴露 **V3.0.1 dp9 NEW popup queue spec wording drift** (vendor `EnqueuePopup` 内 `if (cur=null && !paused) TryShowNextPopup()` → first enqueue cur=null 时立即 show，priority 只影响后续 queue insertion order；P3 cleanup 残留污染 P4/P5) → spike amend (P3 enqueue 顺序对齐 priority DESC + P3/P5 头尾 `ClearAndClosePopupQueue` cleanup) → 第 2 跑 (16:14) 5/5 PASS + 35/35 asserts + push-pop 对称 + 0 unexpected error。**Phase 4 evidence doc** `production/qa/playmode-popup-auto-blocker-2026-05-13.md` NEW ~430 行 8 section (§0 概要 + §1 R3 5 case detail + §2 R2 8/8 closure + §3 AC 10/10 verify + §4 V3.0.1 dp9 NEW closure + dp8 candidate +1 + dp7 NEW 复用 + Type-9 dp1 absorbed + **NEW dp10 candidate** partial class scoped scope 模式 + §5 Sprint 6 Track C insight + §6 Files changed + §7 References + §8 Verdict)。**Phase 5 closure**: story-008 Status: Phase 0+1 readiness-gate-done → ✅ Done + sprint-status.yaml S6-08 + 本 active.md update。Files (12): UIModule.InputBlocker.cs NEW + UIModule.cs amend + S6_08_MockPanels.cs NEW + S6_08_MockPanelsGenerator.cs NEW + 7 prefab + S6-08_PopupAutoBlocker.cs NEW + GameApp.cs amend + DevTestState.cs amend + story-008 amend + EPIC.md amend + sprint-status.yaml (本 entry + top updated) + 本 active.md amend + production/qa/playmode-popup-auto-blocker-2026-05-13.md NEW。Total elapsed 1038ms < 5s budget；total_push_count=8 / total_pop_count=8 / unexpected_error_count=0。**V3.0.1 dp9 NEW governance insight** — Phase 1 readiness gate R2 verify 阶段未完整模拟 vendor `EnqueuePopup` 内 trigger pattern；Phase 3 第 1 跑 R3 fail 暴露；Phase 3 spec amend + spike rewrite 走 V2.0 §V2-1.b 第二轮 verify 路径 closure；ADR-029 V3.x watch list candidate (popup queue spec wording 系列 — 未来 ADR 写 popup queue 行为需细化 first enqueue immediate show vs priority sort distinction)。**NEW dp10 candidate** (UIModule partial class scoped scope 模式) — UIModule.InputBlocker.cs 与 UIModule.PopupQueue.cs 同模式；V3.1 trigger 阈值 partial class file count ≥ 5 时评估 partial class scoping (ShowAPI / CloseAPI / Layer / InputBlocker / PopupQueue 等可能 5+ partial)。**Phase 2/3/4/5 投入** ~3-4 hr (Phase 2 ~1.5-2 hr + Phase 3 第 1 跑 + dp9 closure + 第 2 跑 ~30-45 min + Phase 4 evidence doc ~30-40 min + Phase 5 closure ~15-20 min)。本 session 总时长 Phase 0+1 ~1 hr + Phase 2~5 ~3-4 hr = ~4-5 hr 接近 CLAUDE.md ≤5 hr hard rule 边界；user 决策 [E] 全 Phase 2~5 一气呵成完成。**Sprint 6 Track C 余下 1 story**: S6-04 (chapter 1 error/restart path) Phase 0 R2 verify 起步。**Sprint 7+ ADR-010 InputManager epic 衔接**: 本 story sender-side narrow scope 准备就绪 (5 R3 case 实证 sender 链路 well-formed + 8 push tokens / 8 pop tokens 对称 + 0 unexpected error；listener-side InputBlocker singleton + InputManager class + S5-05 fire-and-forget closure 留 Sprint 7+ ADR-010 InputManager epic 一并)。**Earlier in Session 30 morning-to-afternoon — S6-08 Phase 0 + Phase 1 ✅ READY DONE — story-008-ui-layer-strategy.md 完整 rewrite per V3.0.1 vendor reality compliant + sender-only narrow scope [A]** — Phase 0 R2 vendor reality verify 7 finding 实证 (UIModule.PopupQueue.cs priority DESC + ASC tiebreak / OnSortWindowDepth same-layer sorting / InputBlocker stack + 9 unit test / `Singleton<InputBlocker>` / `class InputManager` 0-production listener-side 留 Sprint 7+ ADR-010 / UIModule.ShowUI/CloseUI/HideUI 全 0-call IInputBlockerEvent 真 deficiency 需补 sender / WindowAttribute UILayer 4 ctor + UIWindow.WindowLayer 是 int / S5-05 NarrativeSequencePlayer IInputBlockerEvent sender precedent line 312/323)；**narrow scope [A] sender-only 决策 ~2 SP** — UIModule.ShowUIImp/CloseUI/HideUI 对 Top(2)/Tips(3) layer panel 自动 fire `OnPushBlocker/OnPopBlocker(token = type.FullName)`；listener-side InputBlocker singleton + InputManager class + S5-05 fire-and-forget closure 留 Sprint 7+ ADR-010 epic 一并 (epic boundary cleanup 复 S6-07 R2.8 [D] mixed strategy precedent)；R3 走 spike subscribe IInputBlockerEvent assert event count + token format pattern (S5-05 P3 listener spy precedent)；**Phase 1 story rewrite** — 从 Sprint 0 placeholder (5 wording drift: ShowWindow/CloseWindow → ShowUI/CloseUI; UILayer Background/HUD/Popup/Overlay/System → vendor Bottom/UI/Top/Tips/System; PauseMenuPanel/ChapterTransitionPanel/HUDPanel/SettingsPanel 假设面板; IInputService.GetBlockerStack 不存在 API; FIFO popup queue → 实际 priority DESC + ASC tiebreak) → V3.0.1 vendor reality compliant + **10 AC** (sender Top/Tips fire + UI/Bottom/System contrast no-fire + popup queue priority + sorting + InputBlocker stack semantic verify-only) + **5 R3 PlayMode probe case** (P1 sender verify + P2 UI/Bottom/System contrast + P3 popup queue priority + push/pop chain + P4 sorting + P5 pause/resume/clear) + **9 R2 表** (R2.1~R2.6 ✅ FULLY; R2.7 ⚠️ DEFERRED Sprint 7+ ADR-010 epic boundary; R2.8 + R2.9 ⚠️ TBD Phase 2) + V3.0.1 Watch List Hooks (Type-9 dp1 closure 应用 R2.6 IInputBlockerEvent fan-out check + Type-5 dp7 NEW reinforce mock fixture protected override + Type-5 dp6 不触发 + Type-8 dp1 可能触发 R3 P3 留观察 + **NEW dp8 candidate** DevTestState `[main-menu]` mode 复用 阈值阶进 V3.1 trigger 候选 spike count 2→3 距阈值 4 仅 1)；**Phase 1 R1+R2+R3 readiness gate verdict ✅ DEFICIENCY-FLAGGED PASS** ready for Phase 2 transition (类似 S6-07 R2 DEFICIENCY-FLAGGED PASS path 实战二次复用)。Files (4): story-008-ui-layer-strategy.md 完整 rewrite + EPIC.md Stories table row 8 amend + Sprint 6 Override block extend S6-08 进展 + sprint-status.yaml S6-08 entry status backlog → phase-1-readiness-gate-done + 本 active.md 同步。Phase 0+1 投入 ~1 hr。**Phase 2 production code defer 下一 session 起** (per CLAUDE.md session ≤5 hr hard rule)：UIModule.cs ~+30-40 行 TryFireInputBlockerPush/Pop helper + 5 mock UIWindow fixture (MockTopPanel + MockTipsPanelA/B/C + MockBottomPanel + MockSystemPanel) ~150-200 行 + S6_08_MockPanelsGenerator.cs Editor + 5 prefab regenerate + S6-08_PopupAutoBlocker.cs spike ~500-600 行 + GameApp RegisterDevSpikes S607→S608 + DevTestState `[main-menu]` mode 扩 V3.0.1 dp8 candidate +1 (HasSpike S5-02 || S6-07 || S6-08)。**Earlier in Session 30 morning — S6-07 ✅ DONE — Phase 2~5 完整实施 + R3 PlayMode 5/5 case + 27/27 asserts + all_passed=true first-attempt-after-DevTestState-fix** — Phase 2 production code: MainMenuPanel.cs ~250 行 (4 button NewGame/Continue placeholder/Settings placeholder/Quit + vendor 7+2 lifecycle 全 protected override + DOTween 0.3s fade-in CanvasGroup OutQuad + BGM hook AudioManager.Instance.PlayMusic('main_menu_bgm', 1.0f) 走 fail-safe Log.Warning per R2.8 [D] closure + ADR-027 §5 named delegate cache 4 field + null-out cleanup) + MainMenuPanelGenerator.cs 4 button + CanvasGroup root alpha=0 + button color/size adjust + MenuItem path Tools/S5-02→S6-07 amend + Assets/Resources/UI/MainMenuPanel.prefab regenerate + S5-02_EndToEndFlow.cs P1 button rename StartChapter1Button → NewGameButton + P5 case [A] delete + S6-07_MainMenuPolish.cs 新建 ~550 行 (1 spike + S607Runtime + S607Tester + 5 R3 case P1 LifecycleVisibilityCompliance + P2 4ButtonWiring + P3 FadeInBgmHookSniffer + P5 QuitButtonClickReflection + P4 NewGameClickDispatch + JSON evidence dump WriteResultJson) + GameApp.cs RegisterDevSpikes S502Spike → S607Spike 切换。Phase 2 中间 1 round DevTestState fix — 第一跑 P4 FAIL delta=0 root cause: DevTestState OnEnter `HasSpike(\"S5-02\")` 仅识别 S5-02 → S6-07 落 [story-001c] 分支 pre-dispatch OnRequestSceneChange(1) → spike RunAllAsync 启动时 chapter 1 已加载 → P4 NewGameButton click chapter1→chapter1 noop；user [A] minimal change → DevTestState `HasSpike(\"S5-02\") || HasSpike(\"S6-07\")` 扩 main-menu mode 复用 + Log scope `[S5-02]` → `[main-menu]` rename (~5 行 change)；第二跑 5/5 case PASS + delta=1 真链 fire 实证。**Phase 4 evidence doc** `production/qa/playmode-main-menu-polish-2026-05-13.md` 8 sections (§0..§8 含 R3 case detail + R2 9/9 closure 表 + AC 10/10 表 + V3.0.1 Watch List Hooks closure + Sprint 6 Track C insight + Files Changed + Verdict)。**Phase 5 closure**: story-006 Status: Phase 0 ✅ + Phase 1 ✅ + Phase 2.0 ✅ → ✅ DONE；sprint-status.yaml + active.md + commit。**Total elapsed 2302ms < 10s perf budget** (per AC-10)；`unexpected_error_count=0` / `bgm_failsafe_warning_captured=true` / `new_game_clicked_log_captured=true`；R2.8 fail-safe Warning 是 R2.8 [D] closure expected 行为不算 unexpected。**ADR-029 governance value**: R2 deficiency-flagged PASS path 实战首次完整跑通 → Phase 1 readiness ⚠️ TBD → Phase 2.0 verify [D] 决策 → Phase 2 production code + R3 ↑ closure 实证 (V2.0 §V2-1.b 路径 S6-07 实战 dp2 candidate)。**V3.0.1 Watch List Hooks NEW dp8 candidate**: DevTestState [main-menu] mode 复用 ⚠️ 留观察 (V3.1 trigger 候选 — main-menu mode spike count >= 4 阈值；当前 2 = S5-02 + S6-07，距阈值 2 个；S6-07 minimal change [A] 已 closure scope 内不需要 V3.1 trigger)。**Sprint 6 Track C 余 2 story**: S6-08 (UI Polish - ErrorPanel) + S6-04 (Tween library setup)。**Earlier in Session 30 morning — S6-07 Phase 2.0 R2 deficiency flag CLOSURE ✅ DONE** — R2.6 ✅ FULLY RESOLVED (`AudioManager : Singleton<AudioManager>, IAudioService` AudioManager.cs:30 → `AudioManager.Instance.PlayMusic(...)` direct call；GameApp.cs:40 Initialize() 在 :55 StartGameLogic() 之前调用 main menu show 时 _isInitialized=true AC-3 facade gate 不 trigger；PlayMusic 内置 fail-safe clipId null/empty + config not found → Log.Warning + return 不抛异常)；R2.8 ✅ DEFICIENCY CLOSURE per [D] mixed strategy — main_menu_bgm clipId grep 0-hit + AudioConfigFromLuban.InitWithDefaults() 仅 chapter1_ambient (id=100) 1 Music entry + InitFromLuban TODO[S5-XX+] stub fallback；user 决策 [D] 遵 epic 边界 Phase 2 production code 不改 AudioConfigFromLuban (UI epic 不 polluted Audio config table)；BGM hook 完整保留 OnRefresh `AudioManager.Instance.PlayMusic(\"main_menu_bgm\", 1.0f)` 走 PlayMusic fail-safe Log.Warning+no-op；新建 Sprint 7+ follow-on backlog story `ui-system-006b: main_menu_bgm asset + Luban entry` 1 SP；R3 P3 case amend mock spy invocation assert (~6 asserts ↑ from 5)。R2 Verdict 升级 **DEFICIENCY-FLAGGED PASS → ✅ FULLY PASS** per ADR-029 V2.0 §V2-1 R2 DEFICIENCY-FLAGGED PASS path → CLOSURE。Files (4): story-006-main-menu.md amend + 新建 story-006b-main-menu-bgm-asset.md + EPIC.md (Stories table +1 row + count 10 → 11 + Sprint 6 Override block) + sprint-status.yaml；Phase 2.0 投入 ~25 min；S6-07 Phase 2 production code (MainMenuPanel.cs S5-02 baseline 142 → 4 button + fade-in + BGM polish + Generator amend + prefab regen + S5-02 spike rename + S6-07 spike + R3 PlayMode probe 5 case + evidence doc + closure) ready 进入下一 step。**Earlier in Session 29 evening late** — S6-07 Phase 1 readiness gate ✅ READY DONE — story-006-main-menu.md 完整 rewrite (~430 lines) per V3.0.1 vendor reality compliant + S6-07 narrow scope [A] 4 button group (NewGame / Continue placeholder / Settings placeholder / Quit) + vendor 7+2 lifecycle `protected override` 强制 + fade-in 0.3s DOTween OutQuad + BGM hook IAudioService.PlayMusic + R3 PlayMode probe 5 case (P1 vendor 7+2 lifecycle visibility modifier compliance + P2 4 button transform.Find + onClick subscribe + P3 fade-in + BGM hook + P4 NewGame onClick → ISceneEvent.OnRequestSceneChange(1) dispatch + P5 Quit onClick → Application.Quit dispatch) + R2 Assumptions Validated 9 items (R2.1-R2.5 + R2.7 + R2.9 ✅；R2.6 IAudioService service locator path + R2.8 main_menu_bgm clipId Luban entry ⚠️ TBD deficiency flag Phase 2 dev-story 起步时强制实证) + V3.0.1 Watch List Hooks (Type-5 dp7 NEW closure / dp6 closure / Type-9 closure / Type-8 不触发 monitoring) + History (Sprint 0 placeholder 11+ wording drift superseded documented)；**R1+R2+R3 readiness gate verdict ✅ READY** (R1 PASS 0 forbidden listener pattern in S5-02 MainMenuPanel.cs baseline + 0 `public override` hit + 2 `protected override` hit per V3.0.1 dp7 NEW + R2 DEFICIENCY-FLAGGED PASS R2.6+R2.8 ⚠️ TBD + R3 5 case spec consistency)；sprint-status.yaml S6-07 status: phase-0-r2-done-pending-main-impl → phase-1-readiness-gate-done-pending-main-impl + file 字段 drift fix (`story-006-main-menu-uiwindow.md` → `story-006-main-menu.md`) + depends_on +S5-02 inline 2 button base + S6-07 Phase 0 dp7 NEW hotfix 全部 list；Phase 1 投入 ~50 min；Phase 2 production code (MainMenuPanel.cs S5-02 baseline 142 行 → 4 button + fade-in + BGM polish ~210-260 行 + MainMenuPanelGenerator amend + prefab regenerate + S6-07_MainMenuPolish.cs spike + GameApp RegisterDevSpikes 切换 + S5-02 spike P5 case rename/delete 评估) defer 下一 session 起 per CLAUDE.md session ≤5 hr hard rule。**Earlier in Session 29 — Sprint 6 Track B governance 100% closure (S6-05 ✅ + S6-06 ✅) + S6-07 Phase 0 R2 verify partial ✅ → V3.0 §V3-1.b Type-5 dp7 NEW visibility modifier drift hotfix** — Phase 0 R2 grep verify vendor source UIBase.cs:144/151/158/165/172/184/197 全 7 lifecycle method + UIWindow.cs:504/509 extra 2 hook 签名实证 `protected virtual void XxxName()` (业务侧 override 必 `protected override`)；surfaced dp7 NEW — **S6-05 commit 45ae96b ADR-011 §G Key Interfaces code block 5 处 `public override` 是 spec amend 自身引入的 wording drift** (R2 grep verify 仅对 method name + lifecycle 顺序，未对 visibility modifier verify vendor source — dp7 NEW reinforce S6-06 V2.0 §V2-1.b R2 增量子条款)；user 决策 [A] 即时 hotfix amend 完成：(1) ADR-011 §G Key Interfaces 5 处 `public override` → `protected override` + §6 lifecycle table visibility column note + §Status / §Date / §Last Verified hotfix marker + §History +1 entry 2026-05-13 evening dp7 NEW；(2) SP-002 line 107 后插入 visibility modifier note (`protected virtual` / `protected override` vendor 实际签名 + S6-05 commit 45ae96b 错引入 hotfix 修正记录)；(3) ADR-029 V3.0 §V3-1.b 表头 'Sprint 5 实战 dp2/dp3/dp4/dp5 + Sprint 6 实战 dp7 NEW (5 dp)' + dp7 NEW row append + 讽刺地 paragraph + 修复模式 Sprint 6 governance maturity note + §V3-5 V4 trigger row 1 Sprint 6 status amend (累计 5 dp 仍 ≤5/sprint 阈值未触发 V4 + 特殊警示 dp7 NEW = spec amend 自身引入 reinforce V2.0 §V2-1.b)；(4) ADR-029 V3.0 → V3.0.1 sub-versioning + Revision History table row + §Date / §Last Verified amend + §History +1 entry 2026-05-13 evening；(5) sprint-status.yaml S6-07 status: ready-for-dev → phase-0-r2-done-pending-main-impl + watch_list Type-5 dp7 NEW data point append；(6) active.md 同步更新。**dp7 NEW 投入** ~25 min (R2 5 + ADR-011 5 + SP-002 5 + V3.0 §V3-1.b/V3-5/Revision History 5 + History/yaml/active 5)；**dp7 NEW governance insight**: ADR amend 工作本身必须遵守 R2 协议自身 (V2.0 §V2-1.b)；spec wording amend ≠ free pass on vendor source verify；任何 spec wording 变更 (尤其 systematic batch amend) 必 vendor source R2 grep verify 每个 modifier (visibility / readonly / virtual / override / abstract / static / async / params 等)。S6-07 main implementation (panel UIWindow 7+2 hook lifecycle + 4 button group + fade-in + BGM start hook + R3 PlayMode probe 5 case + production wiring) 因 session ≤5 hr hard rule defer 下一 session 起。**Earlier in Session 29** — Sprint 6 Track B governance 100% closure: S6-05 ✅ + S6-06 ✅ DONE** — S6-06 ~25 min: ADR-029 V2.0 §V2-1.b R2 增量子条款 "Interface Method Set Fan-out Check" promote 到 V2.0 R2 协议 (Type-9 dp1 closure absorbed into V2.0 R2 协议 — 不引入新 Type-9 drift type pollution，meta-drift 通过 R2 协议自身加强吸收，内在 enforcement 改进胜过新 drift type 标记)；§V3-1.b dp5 cross-link + §V3-5 V4 升级触发条件 row 6 amend；watch_list Type-9 row TRIGGERED → ✅ CLOSED；commit pending。earlier **S6-05 ✅ DONE** — ADR-011 + SP-002 + ADR-014 + ADR-016 + ADR-017 + ADR-028 6 file batch systematic wording amend 单 session 完成 per [A] batch strategy；Phase 1 (SP-002 + ADR-011 UIWindow vendor reality align 18 处 wording delta + R2 grep verify 8 处 audit trail intentionally retained) + Phase 2 (ADR-014 + ADR-016 IPuzzleLockEvent contract align ADR-027 §4 source `OnPuzzleLockAll(PuzzleLockPayload)` + S5-05 production deprecation note per user [A] 决策 — 发现 ADR-014 parameter-less 与 ADR-016 string token 双重 drift from contract source ADR-027；ADR-013 实际不持 IPuzzleLockEvent 定义 — Option A 解读修正为 align ADR-027 source；ADR-027 §4 IPuzzleLockEvent deprecation 单独排 Sprint 7+ polish review 保 batch scope minimal) + Phase 3 (ADR-017 + ADR-028 Amendment 2026-05-09 内容已完整 align — 仅追加 V3.0 §V3-1.b batch closure audit reference + Sprint 6 S6-05 propagation entry)；实际 ~2 hr (40% under 3 hr estimate)；6 file × ~20 wording delta = ~120 spec wording 与 vendor reality 完全对齐；Sprint 6 AI-2 衍生 ✅ closure；ADR governance maturity 显著沉淀。earlier Session 28 — **Sprint 5 retrospective ✅ DONE** — `production/sprints/sprint-5-retrospective.md` 251 行 8 sections + `production/sprint-status.yaml` +sprint_5_summary block + 9 项 Sprint 6 action items 沉淀 (AI-1 ADR-029 V3 Type-5 promote final + AI-2 ADR-011/SP-002/ADR-014-016/-017/-028 systematic wording amend + AI-3 ADR-029 V2.0 §V2-2 R2 grep 完备性子条 + AI-4 S5-07 playtest promote Must Have + AI-5 S5-09 第 3 次 carry hard rule + AI-6 chapter 1 art asset 升级 + AI-7 V3 Type-6/-8/-9 监控 + AI-8 watch list dp ≥2 即立项 cycle 改造 + AI-9 /gate-check pre-production 重跑 + stage.txt 升级)；**Sprint 5 final score: ✅ PASS — ADR-030 §VS Build 第 1 项 100% + ADR-029 V3 governance maturity 显著沉淀** (commit rate 82% / Must Have 100% / chapter 1 happy path 可玩 / V3 Type-5 6 unique dp 强制 promote unlock)；Sprint 4 retro 9 action items 闭环率 7/9 = 77.8%；TODO/FIXME/HACK end 18 (baseline 15 + 20% healthy < 30% threshold)。earlier Session 27 #3 \u2014 **S5-02 /dev-story Phase 2~5 \u5b8c\u6574\u5b9e\u65bd \u2705** \u2014 Chapter 1 end-to-end 5 \u7cfb\u7edf\u4e32\u901a happy path **5/5 R3 PASS + 36/36 asserts + `all_passed=true` first-run after P5 chapter 0 spec drift fix** + total **3844ms** \u226a 10s budget + reflection \u5b9e\u6d4b `GameModule.Audio.MusicVolume=0.300` ducking + 0 unexpected console error/warning; V3 Type-5 **dp6 NEW** = \u7d2f\u8ba1 6 unique dp \u8fdc\u8d85 promote \u9608\u503c (Sprint 5 retro \u5f3a\u5236 promote)\uff1bstory-002 Status **Ready \u2192 Done**\uff1bADR-030 \u00a7VS Build commitment \u7b2c 1 \u9879 **100% \u5b8c\u6210** \u2705\uff1bSprint 5 Track A \u6536\u5b98\u3002\u8be6\u7ec6 sprint \u6570\u636e\u89c1 `production/sprint-status.yaml`)
> Phase: **Sprint 6 in_progress + 上游 template v1.0.0 已合并(本地待 push) — Track B governance 100% closure (S6-05 ✅ + S6-06 ✅) + Track C 100% 收官 (S6-07 ✅ + S6-08 ✅ + S6-04 ✅) + S6-01 Phase 1 prep ✅ + Phase 2.0 playtest infrastructure fix ✅ + Phase 2.1 manual playtest ⚠️ NEEDS-WORK (5 处 chapter 1 production wiring S0 critical surface) + V3.0.1 dp15 candidate NEW 'EditMode green ≠ production wired sniff' (promote 优先级 高于 dp14) — VS Chapter 1 epic 修订: emergent fix Track F NEW story-004~008 ~7-9 hr 必 Sprint 6 本周 split 2-3 daily session 嵌入 (block S6-02 playtest 2 + S6-03 + S6-10 gate-check pre-production stage advance)** ; **Vertical Slice (VS) Sprint 5-6 双 sprint commit Sprint 6 final** — Track B 100% complete + Track C 2/3 done (S6-07 ✅ + S6-08 ✅；S6-04 余下) (start 2026-05-13 / end 2026-05-27 / mode solo / capacity 19 SP commit Must 16 + Should 3 + Nice 3 candidate；9 Must + 1 Should + 2 Nice = 12 stories；Tracks A-E 5 轨：A VS Validation playtest×3 + B ADR Governance Amend ✅ DONE (S6-05 + S6-06) + C UI Polish + Error S6-07/-08/-04 [S6-07 ✅ + S6-08 ✅] + D Art Asset Upgrade S6-09 + E Stage Advance S6-10；ADR-030 §VS Build commit 剩余 3/6 项 Sprint 6 收尾)。Earlier Sprint 5 ✅ CLOSED (retrospective DONE + AI-1 V3.0 promote DONE EARLY Session 28) - Sprint 5 Track A 收官 + Must Have 全 ✅ (start 2026-05-06 / end 2026-05-12 8 day 早收；Track A VS Chapter 1 Build + Track B 三个 P1 ADR production code 全部 ✅ DONE；ADR-030 §VS Build commitment 第 1 项 **100% 完成 ✅**)
> Next milestone: **Track F story-005 R3 PlayMode (S6-14 spike 6 case) + Phase 4 evidence doc + Phase 5 closure** ~50-70 min 估时 — spike `S6-14_ChapterSceneWiring.cs` ✅ written + GameApp S613→S614 + DevTestState HasSpike(S6-14) ✅ committed 2026-06-12 Session 34；待 Unity Editor domain reload + PlayMode 跑 spike → JSON `~/Library/Application Support/DefaultCompany/Unity/S6-14_Result.json` — Phase 2 scene wiring ✅ DONE (本 session ~30 min；unity-mcp batch_execute 完整；Drift D Phase 2 surface Unity engine 2D/3D 互斥约束 → [A2] child Hitbox2D GameObject 路径 narrow scope 同存解；0 production C# diff + 0 ProjectSettings diff)；剩余: (1) spike `S6-14_ChapterSceneWiring.cs` NEW ~400-500 行 1 file + 3 inner class (S614Spike : IDevSpike + S614Runtime + S614Tester per S6-13 precedent) 5+1 R3 case Setup/Action/Assert (P1 SceneHierarchyHasInteractionCoordinator + P2 InteractableObjectsExistAndConfigured + P3 LayerFilterCorrect [含 child Hitbox2D + 父级 3D collider 保留 verify] + P4 CameraReferenceNonNull + P5 RaycastFatFingerDimensionalConsistency [Drift D [A2] sub-decision child GameObject 路径实证] + P5b optional Idempotent) + JSON evidence dump (2) GameApp.cs RegisterDevSpikes S613Spike → S614Spike + (3) DevTestState.cs [main-menu] mode HasSpike list +1 = 7 spike (V3.0.1 dp8 candidate 阈值远超达 7 — Sprint 6 retro 议题 1 强制评估 V3.1 trigger 必上议题) + (4) ReadLints + commit batch (5) Unity Editor R3 PlayMode 跑 spike auto-launch 5+1 case + JSON 验收 (6) Phase 4 evidence doc `playmode-chapter-1-scene-wiring-2026-05-15.md` ~400-500 行 8 sections (per S6-13 precedent) + (7) Phase 5 closure (story-005 Status: Phase 2 partial DONE → ✅ Done + sprint-status.yaml S6-14 status → ✅ done + active.md + commit no push)。**预期 Phase 2 spike + Phase 4 evidence + Phase 5 closure 总投入 ~70-100 min (~下 session 起始)** — 完成后 Track F 6/10 → 7/10；NEXT Track F story-006 GameApp provider injection (~30 min)。 (本 session 接 or 下 session 起始 per [A] override governance debt 第 4 次连续 ≤5 hr hard rule 实际 ~5-5.5 hr 边界 严肃评估)。**Track F story-005 readiness gate verdict ✅ READY (本 session morning 完成)**: R1 ✅ (forbidden listener pattern grep audit 0 hit) + R2 ✅ (cross-component API existence 7/7 line-ref 实证 + 4 drift inline closed via amend per [A] narrow scope + [1] 原位 amend) + R3 N/A auto-pass (Asset/Integration 类型不构造 stub data type) → ADR-029 verdict ✅ PASS。**故障返回路径**: 如 Phase 2 时发现 R2.5 PuzzleConfig 必填字段在 chapter 1 fixture 缺失 → 走 R2 deficiency-flagged PASS path (本 session 已实战 4 次)。**Earlier (本 session 前段)** — Track F story-005 /story-readiness gate 5 gap closure amend ✅ DONE 2026-05-14 night Session 33 morning ~15 min — story-005 amend 7 § (Status header / §GDD Requirement TR-ID drift fix + dp11 第 2 sub-item 触发 / §Engine Notes 末尾 perf budget paragraph / §Test Evidence section NEW Asset/Integration evidence path + R3 spike JSON dump + 0 production C# diff verify + 0 ProjectSettings diff verify / §ADR-029 Verification section NEW R1 + R2 + R3 verdict 显式 / §History entry append) + sprint-status.yaml S6-14 status: phase-0-r2-done-pending-readiness-gate → phase-1-readiness-gate-ready-pending-impl + notes amend + top updated 同步 + 本 active.md 同步；ReadLints clean 0 lint。**Earlier — Track F story-005 /story-readiness gate ✅ DONE 2026-05-14 night Session 33 morning ~15 min before amend** — 27-checklist verdict `⚠️ NEEDS WORK (3 minor + 2 polish)`: TR-ID semantic drift (TR-objint-002/003 不对应 chapter 1 物件挂载 是 design constraint) + unresolved 'TBD verify' wording + perf budget note 缺 + Test Evidence section 缺 + ADR-029 Verification section 缺 → user [A] 推荐 5 项一次性 amend 一次清 → Verdict 升 READY。**Earlier — Track F story-005 Phase 0 R2 verify ✅ DONE 2026-05-14 night (Session 33 morning ~30 min)** — 7/7 R2 evidence + 4 drift verdict A/B/C/D 沉淀 + V3.0.1 dp11 第 2 实战触发 NEW + dp18 candidate NEW 第 1 实战触发 + 3 file amend (story-005 + sprint-status.yaml + active.md) + ReadLints pending + 暂不 commit (留 /story-readiness gate 后一并 commit per user [D] 决策)。**Track F story-005 Phase 2 估时修订 ~1.5-2 hr** (Phase 0 done ~30 min + Phase 1 readiness gate ~30-50 min + Phase 2 implementation ~50-80 min unity-mcp batch_execute add GameObject + Component + 2D collider + Inspector field wire + spike write + ReadLints + Phase 3-5 R3 + evidence doc + closure 留下一 session)。**Track F 余下 4 stories 修订估时 ~5-7 hr** (story-005 ~1.5-2 hr + story-006 ~30 min + story-007 ~2-3 hr scope 风险最高 R2.1 verify 关键 unknown + story-008 ~1 hr final pilot dp15 sniff sub-clause)；replay manual playtest after Track F done (block S6-02 playtest 2 + S6-03 + S6-10 stage advance)。**Earlier — Track F story-005 Phase 0 R2 verify ✅ DONE 2026-05-14 night (Session 33 morning ~30 min)** — 7/7 R2 evidence + 4 drift verdict A/B/C/D 沉淀 + V3.0.1 dp11 第 2 实战触发 NEW + dp18 candidate NEW 第 1 实战触发 + 3 file amend (story-005 + sprint-status.yaml + active.md) + ReadLints pending + 暂不 commit (留 /story-readiness gate 后一并 commit per user [D] 决策)。**Phase 5 closure commit message draft**: `feat(input-pipeline): wire Mouse→FSM→Dispatch production caller (S6-13 story-004 ✅ Done; dp15 sniff sub-clause 试点 第 1 PASS; ADR-010 Step 9 NEW)`。**S6-13 ✅ Done (本 session Phase 5 closure)**: R3 第 2 跑 5/5 PASS + 27/27 asserts + 391ms / dp15 sniff sub-clause 试点 第 1 PASS ⭐ / dp16 closure 第 1 个 / dp11 closure 第 1 个 / dp17 declined / 3 项 deficiency 全 CLOSED / evidence doc DONE / story-004 Status: ✅ Done。 — earlier — **本 session R3 PlayMode 第 1 跑 FAIL diagnostic + SuspendTick fix ✅ DONE (commit pending — InputService.cs +SuspendTick/ResumeTick API + S6-13_InputPipelineWiring.cs RunAllAsync 起步 SuspendTick + try/finally ResumeTick + P2 tapZeroOk 改 baseline delta + ReadLints clean)** — R3 第 1 跑 P1/P3/P4/P5 FAIL (P2 PASS 伪假象) 根因 = `InputService.Tick` 注册 `Utility.Unity.AddUpdateListener` 每帧自动跑，spike `TickForTest` 注入 Began 后 `await UniTask.DelayFrame(1)` 期间 production Tick 调 MouseToTouchAdapter.SampleMouse() 拿真实 mouse 当前位置 → `SingleFingerFSM.ProcessPending` 不查 IsActive 直接计算 frameDist > DragThresholdPx 30.12 → 强制 Pending → Dragging emit Drag.Began (3 phase) 污染所有 Tap case；P2 假 PASS 因 P2 期望 Drag 1 Began noisy drag count 不严格 mismatch。**SuspendTick fix 3 改动**: (1) `InputService.cs` +`_tickSuspended` field + `SuspendTick()/ResumeTick()/IsTickSuspended` public API + Tick() early return guard + Dispose() reset (2) `S6-13_InputPipelineWiring.cs` RunAllAsync 起步 `input.SuspendTick()` + try/finally `inputForResume.ResumeTick()` (异常路径也 resume 保护 manual playtest) (3) `S6-13_InputPipelineWiring.cs` P2 `tapZeroOk` 改 baseline delta (`tapDelta = _allTaps.Count - tapBaseline; tapZeroOk = tapDelta == 0`) 防 P1 PASS 后 P2 sequencing 误判 FAIL；ReadLints clean ✅。**dp17 candidate 'spike harness 与 production update loop 并行帧 noise 污染 spike injection' user [B] decline 沉淀** — discussed and declined per problem-to-rule-promotion 协议；rationale 用户判定 niche (spike harness 隔离 issue 不构成跨多 spike pattern，沉淀 ADR/dp 反成噪声)；如 R3 复跑后第 2 个 spike 再现同样污染则 promote。**NEXT — user 重启 Unity Editor force domain reload (.cs 改动 + 新增文件 .meta 生成) → PlayMode start 跑 spike auto-launch RunAllAsync 5 case → JSON 提供 LLM 验收 (预期 5/5 PASS + 27/27 asserts + 0 unexpected error)**. **Phase 3-5 待办 (after R3 PASS)**: Earlier — user [A] 本 session 立即跑 Unity Editor compile + R3 PlayMode 5/5 case + Phase 4 evidence doc + Phase 5 closure (~30-60 min；本 session 总将达 ~4.5-5 hr 触达 5hr hard rule 边界) / [B] commit 当前 batch (no push) + 本 session 收尾 + 下 session 跑 Phase 3 R3 PlayMode + Phase 4/5 closure / [C] commit + 让 user 跑 R3 PlayMode 后再决定 Phase 4/5 走向。**Phase 3-5 待办 (whichever path)**: Phase 3 Step 1 ReadLints (✅ done) + Unity Editor compile verify (await domain reload — 约 30s-2min) + Phase 3 Step 2 R3 PlayMode 5/5 case execute + JSON evidence dump (~/Library/Application Support/DefaultCompany/Unity/S6-13_Result.json) + Phase 4 evidence doc `production/qa/playmode-input-pipeline-wiring-2026-05-14.md` ~400 行 8 sections (per S6-04/S6-08 precedent — §0 概要 + §1 R3 5 case detail + §2 R2 8/8 closure 表 + §3 AC 10/10 verify + §4 V3.0.1 Watch List Hooks dp15 closure 第 1 个 + dp16 closure 第 1 个 + dp11 closure 第 1 个 + dp8 阈值远超达 + §5 Sprint 6 Track F insight + §6 Files changed 8 项 + §7 References + §8 Verdict) + Phase 5 closure (story-004 Status: DEFICIENCY-FLAGGED PASS → ✅ DONE + sprint-status.yaml S6-13 status: phase-1-readiness-gate-done-deficiency-flagged → ✅ done + EPIC.md story-004 Status update + 本 active.md 同步 + commit no push)。**S6-13 R3 PlayMode 预期 verdict (per readiness gate spec)**: 5/5 case PASS + 27 asserts (P1 6 + P2 5 + P3 6 + P4 4 + P5 6) + 0 unexpected console error/warning + total elapsed < 5s budget；P5 dp15 sniff sub-clause 试点 第 1 个 production caller hit > 0 修复 case verify PASS = V3.0.1 dp15 candidate **正式 promote V3.x sub-version 优先级 +1** (Sprint 6 retro 议题 6 input data point)。**[A] override governance trail (per problem-to-rule-promotion 协议 'discussed and declined' pattern — 用户 explicit override 5hr hard rule 第 3 次连续) ✅ acknowledged**: 用户 explicit decision-maker [A] 选择是 user-driven；governance debt 透明记录 Sprint 6 retro 议题 5 input data point；本 session 实际投入 ≈ 4 hr (~3.5-4 hr 触达 5hr hard rule 边界 但 actual 仍合规)；如 Sprint 7+ ≥1 次再发同模式 → 评估 promote CLAUDE.md hard rule wording amend 或 problem-to-rule-promotion 协议升 cursor rule (跨工程 hard rule 防累积)。**Earlier session continue — S6-13 story-004 Phase 2 implementation 启动 per [A] override CLAUDE.md ≤5 hr hard rule 第 3 次连续 ⚠️ (S6-04 [C] / S6-08 [E] / 本 session [A] 三次连续 hard rule override governance debt 累积 — Sprint 6 retro 议题 5 必讨论)** — Phase 2 implementation 估时 ~3-5 hr；本 session 总将达 ~6.5-8.5 hr (Track F batch ~50 min + readiness gate ~50 min + amend ~30 min + Phase 2 ~3-5 hr)。**[A] override governance trail (per problem-to-rule-promotion 协议 'discussed and declined' pattern — 用户 explicit override 5hr hard rule 第 3 次连续)**: 用户 explicit decision-maker [A] 选择是 user-driven；governance debt 透明记录 Sprint 6 retro 议题 5 input data point；如 Sprint 7+ ≥1 次再发同模式 → 评估 promote CLAUDE.md hard rule wording amend 或 problem-to-rule-promotion 协议升 cursor rule (跨工程 hard rule 防累积) — V3.0.1 dp14/-15/-16 governance debt 已显著高于阈值 + 5hr hard rule 3 连 override = 必经 Sprint 6 retro 处理周期。**Phase 2 implementation 8 file plan**: (1) InputService.cs NEW ~80-120 行 TEngine Module pattern 持有 SingleFingerFSM + DualFingerFSM + InputConfigFromLuban + Update Loop driving FSM via MouseToTouchAdapter (Editor Play only) or Touch sampling (real device) + InputBlocker check + InputFilter check + (2) MouseToTouchAdapter.cs NEW ~50-80 行 Editor Play only `#if UNITY_EDITOR` Mouse → TouchState + (3) GameApp.cs Init amend InitializeInputService 调用 + InitWithDefaults() + (4) S6-13_InputPipelineWiring.cs spike NEW ~500-700 行 5 R3 case (P1~P5) + listener spy + JSON evidence dump + (5) GameApp.cs RegisterDevSpikes S601 → S613 + (6) DevTestState.cs HasSpike S6-13 +1=5 V3.0.1 dp8 阈值远超达 + (7) ADR-010 §Implementation Guidelines amend Step 9 'Editor Mouse Adapter' ~5-10 行 (V3.0.1 dp16 closure 第 1 个) + (8) sprint-status.yaml Sprint 2 SP-013 amend 'DONE → partial-fsm-only-driver-pending-sprint-6-emergent-fix' (V3.0.1 dp11 closure 第 1 个)。**emergent fix epic 完成后 Phase 2.2 replay manual playtest** (reuse session 语义重测，>=30 min fun loop 评估) → replay PASS 后才进 S6-02 playtest 2。**push origin main 累计 12 commits 待 push** (sync v1.0 6 + sync 落档 1 + S6-01 Phase 1 prep 1 + S6-01 Phase 2.0 infra fix 1 + S6-01 Phase 2.1 NEEDS-WORK closure 1 + Track F batch write 1 + S6-13 readiness gate 1) — 用户决定 push 时机。**Sprint 6 retro 议题 +1=7 (评估 NEW 议题 7 候选)**: NEW 议题 7 dp16 candidate 'ADR spec gap re Editor-only path' (与议题 6 dp15 关联评估同根；ADR-010 §Implementation Guidelines 必含 Editor-only path 章节如适用 — 适用 ADR-010 / -011 / -013 / -016 等多 ADR Editor-only path 是否 spec 0 cover；评估 promote V3.x sub-version + ADR 写作 standard amend；与 dp14 关联 dp16 同 Editor 工具链层 但角度不同 — dp14 PlaytestSpike pattern Phase 1 prep 工具链 gap；dp16 ADR Editor-only path spec gap)。**Sprint 6 retro 议题 5 amend** (CLAUDE.md ≤5 hr hard rule override governance debt 反思): 第 3 次连续 [A]/[C]/[E] override (S6-04 [C] override 总 ~7-8 hr + S6-08 [E] 一气呵成总 ~4-5 hr + 本 session [A] override 总将达 ~6.5-8.5 hr) — 评估 (a) hard rule 边界 wording amend (≤5 hr → ≤6 hr 或 case-by-case 软规则) (b) hard rule override governance trail standard (每次 override 必 explicit user signoff + commit message 强制 'governance debt' marker + retro 议题强制添加) (c) hard rule override 阈值 (≥3 次连续 override → 强制 retro stop the line)。**Sprint 6 retro 议题 6 项 (dp15 NEW promote 优先级 高于 dp14)**: (1) V3.0.1 dp8 candidate 阈值 5 (>4) 触达 + V3.1 trigger 决策 (2) NEW dp14 Type-10 candidate 'playtest infrastructure pattern gap' 评估 ADR-029 V3 promote + standard PlaytestMode pattern doc + 与 dp8 关联是否合并 V3.1 trigger 提案 (3) /vertical-slice vs S6-10 sequencing (4) dp13 候选 Director gates vs ADR-029 V3 coexistence (5) /prototype 双模式 vs ADR-030 §VS Build commit + ≤5 hr hard rule 2 次 override 反思 ⭐ (6) **NEW dp15 Type-11 candidate 'EditMode green ≠ production wired sniff' (promote 优先级 高于议题 5 dp14)** — 评估 promote V3.x sub-version + ADR-029 V3 R3 standard amend "production caller hit > 0 sniff sub-clause" + 评估与 dp14 关联 (同根 'spike/test PASS 不等于 production 可玩'反模式 但层级不同) + 评估应用范围 (ShadowMatchCalculator / NarrativeSequencePlayer / ChapterStateManager / 未来 InputManager epic 等全部跨 sender → listener 链路系统) + 评估与 dp9/dp14 关联 cumulative governance debt (三 dp 同根 Phase 1 readiness gate 完备性反复出现)。**Sprint 6 余下 Track 主路径序列 (修订)**: S6-01 Phase 2.1 ⚠️ NEEDS-WORK → **emergent fix epic Track F NEW story-004~008 ~7-9 hr** → S6-01 Phase 2.2 replay manual playtest (PASS expected) → emergent fix S1/S2 polish + S6-09 art asset 升级 (Should，依 [A2] replay 后再评估) → S6-02 playtest 2 → emergent fixes → S6-03 playtest 3 + report → **S6-10 /gate-check pre-production + production/stage.txt: Pre-Production → Production formal advance**。如 emergent fix 总投入 >> ~7-9 hr 估时 → Sprint 7 buffer 触发；本周 Sprint 6 末 ≤2026-05-27 截止。**Sprint 6 retro 必讨论议题**: (1) V3.0.1 dp8 candidate (DevTestState [main-menu] mode 复用) 阈值 4 触达 — promote V3.1 trigger pattern (`IDevSpike.IsMainMenuMode` flag + DevTestState central mode-dispatch refactor ~30-50 行) vs 保持显式 HasSpike list；(2) V3.0.1 NEW dp11 candidate (sprint backlog placeholder wording drift) closure 总结 — 未来 Sprint backlog placeholder 写入时 R2 grep verify vendor source 优先级提升；(3) V3.0.1 NEW dp12 candidate (isolated local SceneManager + global GameEvent collateral 首次实战) — ADR-009 V3.x watch list candidate (spike isolated local SceneManager 文档化 "global GameEvent vs local listener" 区分守则)；(4) **CLAUDE.md ≤5 hr hard rule 2 次连续 [C] override 反思** (S6-04 实施总 session 投入 ~7-8 hr — governance debt 累积；评估是否调整 hard rule 边界 + 何时强制 session 收尾)；(5) Track B 余下评估 (Track D art asset 升级 / Track E stage advance 待启动)。**S6-08 ✅ DONE (本 session 收尾)**: Phase 2~5 完整实施 + V3.0.1 dp9 NEW spec wording amend + R3 5/5 case PASS + 35/35 asserts + Push=8/Pop=8 对称 + 0 unexpected error；commit pending (UIModule.InputBlocker.cs NEW + UIModule.cs amend + S6_08_MockPanels.cs NEW + S6_08_MockPanelsGenerator.cs NEW + 7 prefab + S6-08_PopupAutoBlocker.cs NEW + GameApp.cs + DevTestState.cs + story-008 + EPIC.md + sprint-status.yaml + active.md + playmode-popup-auto-blocker-2026-05-13.md NEW 一起进 1 commit)。Sprint 6 Tracks A 主路径序列：S6-06 → S6-07 ✅ → S6-08 ✅ → S6-04 → S6-01 playtest 1 → emergent fixes + S6-09 art asset 升级 (Should) → S6-02 playtest 2 → emergent fixes → S6-03 playtest 3 + report → **S6-10 /gate-check pre-production + production/stage.txt: Pre-Production → Production formal advance (Sprint 6 收官 ADR-030 §VS Build commit final 项)**。如 gate-check FAIL → Sprint 7 buffer。Sprint 7+ ADR-010 InputManager epic 衔接 ready (本 S6-08 sender-side narrow scope 已 well-formed — 5 R3 case 实证 sender 链路 + listener-side InputBlocker singleton + InputManager class + S5-05 fire-and-forget closure 留 Sprint 7+ ADR-010 InputManager epic 一并)。
> **S5-02 Session 27 #3 Phase 3 第 1 跑 F4 发现 + 修复**:
>   - **F4 P5 chapter 0 spec drift** \u2014 `ISceneEvent.OnRequestSceneChange(0)` \u4e0d\u88ab\u652f\u6301 (API \u4ec5 documented support `targetChapterId in [1, 5]`)；vendor SceneManager \u5bf9 id=0 \u8d70 `TryResolveOrFail` \u2192 `OnSceneLoadFailed` \u2192 `state=Error` \u2192 DriveTransitionAsync \u4e0d\u88ab\u8c03\uff1bchapter 1 \u4e0d\u88ab unload\u3002\u4fee\u590d\u8def\u5f84 [A] (Session 27 #3 user)\uff1a`GameApp.BuildFixtureChapterDataProvider()` \u6269\u5c55 chapter 2 placeholder fixture (sceneId=Chapter_01_Approach MVP simplification \u590d\u7528) + `MainMenuPanel.OnNextChapterButtonClicked()` \u6d3e `OnRequestSceneChange(2)` \u8d70 chapter 1 \u2192 chapter 2 \u771f\u5207\u6362 + spike P5 case \u6539\u540d NextChapterButtonSwitchToChapter2 + Assert \u6539 `CurrentLoadedChapterId == 2`\u3002\u6c89\u6dc0 V3 Type-5 dp6 NEW = \u7d2f\u8ba1 6 unique dp\u3002\u771f main menu return \u8def\u5f84 (chapter unload \u4e0d reload) \u7559 Sprint 6 polish \u2014 \u9700\u5148\u6269 ISceneEvent spec \u6216\u65b0\u589e IGameFlowEvent.OnReturnToMainMenu\u3002
> **S5-02 Phase 3 PASS \u603b\u89c8 (Session 27 #3 \u7b2c 2 \u8dd1)**: P1 MainMenuButtonBootChapter1 11/11 asserts 47ms + P2 ObjectInteractionToShadowMatch 5/5 asserts 7ms + P3 PuzzleStateTransitionToComplete 6/6 asserts 7ms + P4 NarrativeSequenceWithAudioDuck 7/7 asserts 2701ms (**reflection \u5b9e\u6d4b `GameModule.Audio.MusicVolume=0.300` \u2705**) + P5 NextChapterButtonSwitchToChapter2 7/7 asserts 269ms (chapter 1 unload + chapter 2 loaded + state=Idle)\u3002Total **3844ms \u226a 10000ms budget** \u2705\u3002Evidence doc: `production/qa/playmode-end-to-end-flow-2026-05-12.md` 8 sections (\u00a70..\u00a78 \u542b AC matrix 10/10 + V3 Watch List Hooks dp6 NEW + Sign-off)\u3002
> **S5-02 V3 dp \u7ec8\u6001 (Session 27 #3)**:
>   - V3 Type-5 'spec \u2194 reality drift' = S5-01 dp1 + S5-08 dp2 + S5-08 dp3 + S5-02 dp4 (Session 27 #1) + S5-02 dp5 (Session 27 #2) + **S5-02 dp6 NEW (Session 27 #3 ISceneEvent chapter 0 spec drift)** = **6 unique dp** \u8de8 3 story / 4 \u5b50\u7cfb\u7edf (toolchain / UIModule / IShadowEvent / ISceneEvent) \u2192 Sprint 5 retro **\u5f3a\u5236 promote** ADR-029 V3 \u6b63\u5f0f candidate
>   - V3 Type-2(c) dp 0 (Phase 3 0 \u65b0\u89e6\u53d1)
>   - V3 Type-9 'R2 grep \u5b8c\u5907\u6027' meta-drift dp1 (Session 27 #2)\u7559\u89c2\u5bdf 1-2 story
> **S5-02 Production code changes (Session 27 #3)**:
>   - `DevBootstrap.cs` +`HasSpike(string id)` helper (+15 -0)
>   - `DevTestState.cs` OnEnter \u52a0 S5-02 \u5206\u652f `RunRequested() \u2192 ShowMainMenuPanelAsync()` (+44 -10)
>   - `GameApp.cs` RegisterDevSpikes S508Spike \u2192 S502Spike + BuildFixtureChapterDataProvider \u6269\u5c55 chapter 2 placeholder (+22 -7)
>   - \u65b0\u5efa `MainMenuPanel.cs` 142 \u884c (production UIWindow + 2 Button onClick \u2192 ISceneEvent.OnRequestSceneChange dispatch)
>   - \u65b0\u5efa `MainMenuPanelGenerator.cs` 182 \u884c (Editor MenuItem `Tools/S5-02/Generate Main Menu Panel Prefab`)
>   - \u65b0\u5efa `Assets/Resources/UI/MainMenuPanel.prefab` (Generator \u751f\u6210)
>   - \u65b0\u5efa `Spikes/S5-02_EndToEndFlow.cs` 935 \u884c (1 \u6587\u4ef6 + 3 \u5185\u7c7b\uff1bSpike + Runtime + Tester per S5-1b/1c/-08 precedent；Awake() \u540c\u6b65 subscribe early listeners + InitializeProductionWiring \u5185 `new PuzzleStateMachine + new NarrativeSequencePlayer` spike \u4e34\u65f6\u6258\u7ba1\u3001\u7559 Sprint 6 ChapterStateManager \u63a5\u5165)
>
> **\u4e0a Session 27 #2 status (\u4fdd\u7559 audit trail)**: dev-story Phase 1.5 self-check 3 \u91cd\u5927\u53d1\u73b0 per \u51b3\u7b56 [A]\u00d73 \u2014 F1 R2.2 wiring gap \u8bef\u5224\u64a4\u56de + F2 R2.3 chapter 1 \u7269\u4ef6 RESOLVED + F3 `OnPerfectMatchEnter`\u2192`OnPerfectMatch` wording drift propagate fix\uff1bR2 verdict DEFICIENCY-FLAGGED PASS \u2192 \u2705 PASS\uff1bV3 Type-5 dp5 NEW \u7d2f\u8ba1 5 unique dp + V3 Type-2(c) dp1 RETRACTED + V3 Type-9 dp1 NEW 'R2 grep \u5b8c\u5907\u6027 meta-drift'\u3002
> **S5-02 Phase 1.5 self-check (Session 27 #2 per \u51b3\u7b56 [A]\u00d73)**:
>   - **F1 R2.2 wiring gap \u8bef\u5224\u64a4\u56de** \u2014 Session 27 #1 grep \u4ec5\u67e5 `OnPuzzleComplete` listener \u5f97 0-hit\uff0c\u4f46 NarrativeSequencePlayer.cs:**133-135** \u5b9e\u9645 4 \u4e2a listener (`OnRequestSequence` + **`OnPerfectMatch`** + `OnAbsenceAccepted` + `OnChapterComplete`)\uff1b`OnPerfectMatchHandler` (line 239-242) \u2192 `HandleRequestSequence(puzzleId, NarrativeSequenceType.MemoryReplay)` \u2192 puzzle\u2192narrative chain \u76f4 wire \u2705\u3002`ChapterStateManager` Sprint 5 production 0-caller \uff08`new ChapterStateManager(...)` 0-hit / `ChapterStateEventBridge.Attach(...)` 0-hit\uff09 \u2014 chapter \u63a8\u8fdb\u903b\u8f91 deferred to Sprint 6+\uff0chappy path \u4e0d\u4f9d\u8d56\u3002\u53d6\u6d88 PuzzleCompleteToNarrativeBridge.cs \u5b9e\u65bd\u3002
>   - **F2 R2.3 chapter 1 \u7269\u4ef6 RESOLVED** \u2014 unity-mcp `manage_scene get_hierarchy parent=Interactables` \u5b9e\u6d4b\uff1a`Object_01_CoffeeMug` componentTypes `[Transform, MeshFilter, BoxCollider, MeshRenderer]`\uff1b`Object_02_Book` `[Transform, MeshFilter, CapsuleCollider, MeshRenderer]` \u2014 \u7f3a `InteractableObject` MonoBehaviour\u3002\u4f46 P2 Action \u5df2\u8bbe\u8ba1\u4e3a spike fire mock `OnMatchScoreUpdated(1, 0.95)` \u7b80\u5316\u8def\u5f84\uff0c\u4e0d\u4f9d\u8d56 IO FSM\uff1b\u4e0d\u8865 component\uff0cInteractableObject + puzzle config injection + ADR-012 ShadowMatchCalculator \u7559 Sprint 6 polish\u3002
>   - **F3 `OnPerfectMatchEnter` \u2192 `OnPerfectMatch` wording drift propagate fix** \u2014 `IShadowPuzzleEvent.cs:39` \u5b9e\u4e3a `void OnPerfectMatch(int puzzleId, float finalMatchScore);`\uff08\u65e0 Enter \u540e\u7f00\uff0c\u989d\u5916\u5e26 `float finalMatchScore`\uff09\uff1bstory-002 4 \u5904\u9519\u5199 propagate fix (Engine Notes / Control Manifest Rule / AC-4 / R3 P3 case / JSON evidence schema)\u3002
>   - **\u91cd\u8bc4 verdict**: R1 \u2705 PASS (inherit Session 27 #1) + **R2 \u91cd\u8bc4 \u2705 PASS** (R2.2 \u2705 + R2.3 \u2705 RESOLVED + F3 \u26a0\ufe0f wording drift propagate fix\uff1b8/9 \u2705 + 1/9 F3 fix \u540e \u2705) + R3 \u2705 propagate fix\u3002\u4e3b verdict **READY \u4e0d\u53d8**\u3002
> **S5-02 V3 dp \u8c03\u6574 (Session 27 #2)**:
>   - V3 Type-5 'spec \u2194 reality drift' **dp5 NEW** = S5-01 dp1 + S5-08 dp2 + S5-08 dp3 + S5-02 dp4 + **S5-02 dp5 NEW (OnPerfectMatch[Enter] wording)** = **5 unique dp** \u8fdc\u8d85 promote \u9608\u503c (\u22653) \u2192 Sprint 5 retro **\u5f3a\u5236 promote** ADR-029 V3 \u6b63\u5f0f candidate
>   - V3 **Type-2(c) wiring gap dp1 RETRACTED** (\u539f Session 27 #1 \u8bef\u5224\uff1bSession 27 #2 \u91cd\u67e5\u53d1\u73b0 OnPerfectMatch \u76f4 wire \u5df2\u8db3\u591f\uff1bADR-014 \u00a7G + ADR-016 \u00a7G \u672a\u660e\u793a bridge \u8d23\u4efb\u662f\u4e8b\u5b9e\uff0c\u4f46\u672c story \u4e0d\u4f9d\u8d56\u63a5\u5165\uff0c\u7559 Sprint 6 ChapterStateManager production \u63a5\u5165\u65f6\u91cd\u8bc4)
>   - V3 **Type-9 dp1 NEW** 'R2 grep \u5b8c\u5907\u6027 meta-drift' (Session 27 #1 R2.2 grep \u4ec5\u67e5 `OnPuzzleComplete` listener\uff0c\u672a\u67e5 `IShadowPuzzleEvent` \u5176\u4ed6 method (`OnPerfectMatch` etc) listener \u2192 \u8bef\u5224 wiring gap\uff1b\u63d0\u8bae ADR-029 V2.0 \u00a7V2-2 R2 \u5b50\u6761\u8865 'R2 grep \u9700\u5148 read \u63a5\u53e3\u6587\u4ef6 \u2192 list method \u96c6 \u2192 grep \u6bcf method listener \u96c6'\uff1b\u7559\u89c2\u5bdf 1-2 story \u540e promote)
>   - \u6c89\u6dc0 lessons memo: `.claude/memory/problem_2026-05-12_r2-grep-completeness-interface-method-set.md` (Type-9 \u539f\u578b)

> **S5-02 readiness gate (Session 27 #1 per 决策 [A] \u2014 \u539f\u59cb\u7eaa\u5f55)**: R1 ✅ PASS (0 forbidden listener pattern) / R2 DEFICIENCY-FLAGGED PASS \u2192 \u2705 PASS (Session 27 #2 \u91cd\u8bc4) \u2014 R2.1 ⚠️ WORDING DRIFT 5 处 propagate fix + R2.2 \u26a0\ufe0f WIRING GAP \u2192 \u2705 RETRACTED (Session 27 #2 \u8bef\u5224) + R2.3 \u26a0\ufe0f TBD \u2192 \u2705 RESOLVED (Session 27 #2 unity-mcp \u5b9e\u67e5 + spike \u7b80\u5316\u8def\u5f84) + R2.4/R2.5/R2.6/R2.7/R2.8 ✅ PASS / R3 ✅ propagate fix (P2/P3/P4 case Assert wording + JSON schema events 同步修复)\u3002Story Status: Draft \u2192 Ready (Session 27 #1) \u2192 Ready unchanged (Session 27 #2 amend R2 verdict)\u3002
> **S5-08 sign-off (Session 26 #6 per /dev-story Phase 2~4)**: PlayMode 4/4 case PASS first-run (P1 UIRoot scene 实例化 verify + UILayerExt 0/100/200/300/400 + GameModule.UI Singleton ✅ / P2 ShowUI Mock panel via vendor [Window] attribute + Resources.Load + UIWindow.cs 强制 Canvas/GraphicRaycaster 验证 ✅ / P3 7+2 lifecycle vendor 完整调用顺序 first show frame=30 → visible 帧 → CloseUI<T>() OnDestroy → second show frame=59 完整 init replay [V3 Type-8 dp1 NEW 触发] ✅ / P4 Button.onClick.Invoke() ClickCount++ 通路 ✅) + 29/29 asserts + JSON evidence dump `~/Library/Application Support/DefaultCompany/Unity/S5-08_Result.json` 2026-05-11 18:20:35 + 0 unexpected console error/warning + 0 vendor patch + AC matrix 10/10 PASS。Evidence doc: `production/qa/playmode-uimodule-setup-2026-05-11.md` ~280 行。Phase 2 实施 4 C# files (UILayerExtensions.cs / S5_08_MockMinimalPanel.cs / S5-08_UIModuleSetup.cs / S5_08_MockPanelGenerator.cs Editor) + 1 prefab (Assets/Resources/UI/S5_08_MockMinimalPanel.prefab) + GameApp.cs RegisterDevSpikes 切 S51cSpike → S508Spike。Phase 2.3 中间 1 round fix: prefab 首次缺 Canvas + GraphicRaycaster → vendor UIWindow.cs:484 强制 → Editor 脚本 amend root.AddComponent。
> **S5-08 V3 dp 现状 (Session 26 #6 总览)**: V3 Type-5 'spec/tooling ↔ reality drift' 3 unique dp (dp1 S5-01 + dp2 S5-08 #4 + dp3 S5-08 #5) 超 promote 阈值 → Sprint 5 retro 强烈建议 promote + V3 **Type-8 NEW** 'UIWindow second show 行为 spec vs vendor drift' 1 dp NEW (S5-08 #6 R3 P3 实战) — vendor 销毁后重建 (destroy-and-recreate) 模式 ≠ spec 假设 隐藏后重显示 (hide-and-reshow) 模式 → ADR-011 §G + SP-002 系统性 amend 高优 Sprint 5 retro action item。
