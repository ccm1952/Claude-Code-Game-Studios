...
| **S5-08** | **UI Module Setup (promoted Must Have 2026-05-11; Sprint 5 narrow scope amendment 2026-05-11)** | Integration | **2 SP** | S3-08 ✅ origin + ADR-011 ✅ + ADR-029 V2.0 ✅ + S5-04 ✅ | 2026-05-11 Session 26 #3 narrow scope amendment per 决策 [D']。story file (production/epics/ui-system/story-001-uimodule-setup.md) Draft 重写 81 → 312 行 / 12 sections。本 sprint 范围 narrow scope (10 AC + R3 PlayMode probe 4 case): UILayer 枚举 + UIRoot Canvas runtime 实例化 + GameModule.UI 通路 + ShowWindow/CloseWindow API + Mock minimal panel lifecycle verify (S5_08_MockMinimalPanel DevTest fixture) + Button onClick path verify + UIWindow lifecycle 文档注释 + UIModule 程序集路径注释。**Out of Scope**: Popup Queue Manager / Auto-Dequeue / Auto InputBlocker / Overlay limit / 双 InputBlocker 叠加 → 已由 ui-system-008 cover (9 AC + 5 TC) Sprint 6 polish (不新建 ui-system-001b)。Full Main Menu UIWindow (New Game / Continue / Settings / 存档检查 / fade-in / BGM) → ui-system-006 Sprint 6 polish (S5-02 内 2 minimal inline Button 基于本 story API 通路实施)。S5-08 dev-story DONE 后 unblocks S5-02 hard prerequisite (per Sprint 5 [A] serial 序列：S5-04 ✅ → S5-08 → S5-02 → S5-07)。R3 PlayMode probe 4 case (P1 UIRootCanvasRuntimeInit / P2 ShowMockPanelToUIRoot / P3 UIWindowLifecycleOrder / P4 ButtonOnClickPath) — M1 dual-layer 全 production reflection；spike Awake() subscribe (per S5-1c lessons memo)；spike Assets/.../S5-08_UIModuleSetup.cs 1 file + 3 inner class per S5-1b/1c precedent。Sprint 4 retro action #3 hard rule (第 2 次 carryover promote/descope) 通过 promote (narrow scope) + descope rationale (Popup/InputBlocker robust UI infrastructure → story-008) 双轨满足。|
