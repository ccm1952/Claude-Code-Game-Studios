...
- PopupQueue弹窗队列：UIModule的partial扩展（UIModule.PopupQueue.cs），独立于_uiStack管理弹窗排队逻辑，支持优先级、单实例复用、暂停/恢复/清空
...
- 关闭与隐藏：CloseUI/HideUI，支持隐藏延时自动关闭；CloseUI/HideUI末尾通过OnPopupClosed钩子通知弹窗队列
- 窗口栈管理：Push/Pop/CloseAll/CloseAllWithOut，维护窗口顺序与层级
- 弹窗队列：EnqueuePopup按优先级入队，TryShowNextPopup弹出下一个，PausePopupQueue/ResumePopupQueue/ClearPopupQueue控制队列状态
...
- 弹窗队列（PopupQueue）
  - 独立于_uiStack的弹窗队列系统，排队中的弹窗不进入_uiStack，轮到时才调用ShowUI
  - 实现文件：[UIModule.PopupQueue.cs](file://Assets/GameScripts/HotFix/GameLogic/Module/UIModule/UIModule.PopupQueue.cs)
  - 支持数字优先级（值越大越优先）、同优先级按添加顺序（FIFO）
  - 同类型窗口连续弹出时复用实例，通过OnRefresh + 新UserDatas刷新界面
  - CloseUI/HideUI内部通过OnPopupClosed钩子自动触发下一个弹窗
  - 与手动ShowUI互不影响
  - 核心API：EnqueuePopup/PausePopupQueue/ResumePopupQueue/ClearPopupQueue/ClearAndClosePopupQueue
  - 查询属性：PopupQueueCount/IsPopupQueuePaused/HasActivePopup