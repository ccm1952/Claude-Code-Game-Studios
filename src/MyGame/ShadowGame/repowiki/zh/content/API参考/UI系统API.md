...
- [UIModule.PopupQueue.cs](file://Assets/GameScripts/HotFix/GameLogic/Module/UIModule/UIModule.PopupQueue.cs)
...
- 弹窗队列（PopupQueue）
  - EnqueuePopup<T>(priority, userDatas)/EnqueuePopup(type, priority, userDatas)：将弹窗加入队列，按优先级排序。
  - PausePopupQueue/ResumePopupQueue：暂停/恢复队列弹出。
  - ClearPopupQueue：清空队列（不关闭当前弹窗）。
  - ClearAndClosePopupQueue：清空队列并关闭当前弹窗。
  - PopupQueueCount/IsPopupQueuePaused/HasActivePopup：查询队列状态。
- 常量
  - LAYER_DEEP/WINDOW_DEEP/WINDOW_HIDE_LAYER/WINDOW_SHOW_LAYER：层级与层位常量。

章节来源
- [UIModule.cs:15-735](file://Assets/GameScripts/HotFix/GameLogic/Module/UIModule/UIModule.cs#L15-L735)
- [UIModule.PopupQueue.cs:1-183](file://Assets/GameScripts/HotFix/GameLogic/Module/UIModule/UIModule.PopupQueue.cs#L1-L183)
